...
require_line "$TMP_DIR/tenant-tcp-ipv4.yaml" "    kubeProxy: {}"
...
require_line "$TMP_DIR/tenant-tcp-ipv6.yaml" "    kubeProxy: {}"
...
require_line "$TMP_DIR/tenant-tcp-dual.yaml" "    kubeProxy: {}"
...
require_text "$SCRIPT" "patch_tenant_kube_proxy_command"
require_text "$SCRIPT" "--conntrack-max-per-core=0"
require_text "$SCRIPT" "--conntrack-min=0"
