...
func getValueFromSecret(cs clientset.Interface, namespace, secretName string, fieldName string) (string, error) {
...
var _ = framework.SerialDescribe("[group:ipsec-cert-mgr]", func() {
	...
	framework.ConformanceIt("Should keep working when rotating CA", func() {
		...
		for _, pod := range podList.Items {
			framework.WaitUntil(0, time.Second*30, func(ctx context.Context) (bool, error) {
