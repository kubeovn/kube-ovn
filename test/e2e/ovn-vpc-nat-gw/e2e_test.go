...
var _ = framework.Describe("[group:ovn-vpc-nat-gw]", func() {
	...
	framework.ConformanceIt("Test ovn eip fip snat dnat", func() {
		...
		nbLbOutput, nbStderr, nbErr := framework.NBExec("ovn-nbctl lr-lb-list " + noBfdVpcName)
		framework.ExpectNoError(nbErr, "failed to list lbs for vpc %s: %s", noBfdVpcName, string(nbStderr))
		nbLbList := string(nbLbOutput)
		framework.Logf("OVN LB list for vpc %s:\n%s", noBfdVpcName, nbLbList)
		framework.ExpectContainSubstring(nbLbList, multiDnat1Name)
		framework.ExpectContainSubstring(nbLbList, multiDnat2Name)

		ginkgo.By("Verifying backend OVN LB VIPs match CRD status")
		nbLbDetail, nbStderr, nbErr := framework.NBExec("ovn-nbctl lb-list", "--columns=name,vips", "--format=json")
		...
		framework.ExpectContainSubstring(nbLbDetailStr, multiDnat1.Status.V4Eip+":80")
		framework.ExpectContainSubstring(nbLbDetailStr, multiDnat2.Status.V4Eip+":80")
		...
		ginkgo.By("Verifying backend OVN: first dnat lb removed, second dnat lb still present")
		nbLbOutput, nbStderr, nbErr = framework.NBExec("ovn-nbctl lr-lb-list " + noBfdVpcName)
		framework.ExpectNoError(nbErr, "failed to list lbs for vpc %s: %s", noBfdVpcName, string(nbStderr))
		nbLbList = string(nbLbOutput)
		framework.Logf("OVN LB list for vpc %s after deleting first dnat:\n%s", noBfdVpcName, nbLbList)
		framework.ExpectNotContainSubstring(nbLbList, multiDnat1Name)
		framework.ExpectContainSubstring(nbLbList, multiDnat2Name)

		ginkgo.By("Verifying backend OVN: first dnat removed, second dnat still present")
		nbDnatOutput, nbStderr, nbErr := framework.NBExec("ovn-nbctl lr-nat-list " + noBfdVpcName)
		framework.ExpectNoError(nbErr, "failed to list nat rules for vpc %s: %s", noBfdVpcName, string(nbStderr))
		nbDnatList := string(nbDnatOutput)
		framework.Logf("OVN NAT list for vpc %s after deleting first dnat:\n%s", noBfdVpcName, nbDnatList)
		framework.ExpectNotContainSubstring(nbDnatList, multiDnat1.Status.V4Eip)
		framework.ExpectContainSubstring(nbDnatList, multiDnat2.Status.V4Eip)
		...
		framework.ExpectNoError(nbErr, "failed to list lbs for vpc %s: %s", noBfdVpcName, string(nbStderr))
		nbLbList = string(nbLbOutput)
		framework.Logf("OVN LB list for vpc %s after deleting second dnat:\n%s", noBfdVpcName, nbLbList)
		framework.ExpectNotContainSubstring(nbLbList, multiDnat1Name)
		framework.ExpectNotContainSubstring(nbLbList, multiDnat2Name)
