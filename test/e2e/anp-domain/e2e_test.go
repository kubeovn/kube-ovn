...
var _ = framework.SerialDescribe("[group:admin-network-policy]", func() {
	...
	framework.ConformanceIt("should handle multiple ANPs sharing the same domainName with reference counting", func() {
		f.SkipVersionPriorTo(1, 15, "AdminNetworkPolicy domainName support was introduced in v1.15")
		...
		// Test connectivity to baidu.com before applying any ANP (should succeed)
		testNetworkConnectivity("https://www.baidu.com", true, "Testing connectivity to baidu.com before applying any ANP (should succeed)")
		...
		anp1 := framework.MakeAdminNetworkPolicy(anpName, 50, namespaceSelector, []netpolv1alpha1.AdminNetworkPolicyEgressRule{egressRule1}, nil)
		...
		// Test connectivity to baidu.com after applying first ANP (should be blocked)
		testNetworkConnectivity("https://www.baidu.com", false, "Testing connectivity to baidu.com after applying first ANP (should be blocked)")

		ginkgo.By("Creating second AdminNetworkPolicy with the same domainName to deny baidu.com")
		egressRule2 := framework.MakeAdminNetworkPolicyEgressRule("deny-baidu-2", netpolv1alpha1.AdminNetworkPolicyRuleActionDeny, ports, domainNames)
		anp2 := framework.MakeAdminNetworkPolicy(anpName2, 51, namespaceSelector, []netpolv1alpha1.AdminNetworkPolicyEgressRule{egressRule2}, nil)
		...
		// Test connectivity to baidu.com after applying both ANPs (should still be blocked)
		testNetworkConnectivity("https://www.baidu.com", false, "Testing connectivity to baidu.com after applying both ANPs (should still be blocked)")

		ginkgo.By("Verifying both ANPs have the same domainName rule")
		// Verify first ANP
		framework.ExpectEqual(len(anp1.Spec.Egress), 1)
		anp1EgressRule := anp1.Spec.Egress[0]
		framework.ExpectEqual(len(anp1EgressRule.To), 1)
		...
		peer2 := anp2EgressRule.To[0]
		framework.ExpectEqual(len(peer2.DomainNames), 1)
		framework.ExpectEqual(string(peer2.DomainNames[0]), "*.baidu.com.")

		// Delete the first ANP (baidu.com should still be blocked by the second ANP)
		ginkgo.By("Deleting first AdminNetworkPolicy " + anpName)
		anpClient.Delete(anpName)

		// Test connectivity after deleting first ANP (should still be blocked by second ANP)
		testNetworkConnectivity("https://www.baidu.com", false, "Testing connectivity to baidu.com after deleting first ANP (should still be blocked by second ANP)")

		// Delete the second ANP (now baidu.com should be allowed)
		ginkgo.By("Deleting second AdminNetworkPolicy " + anpName2)
		anpClient.Delete(anpName2)

		// Test connectivity after deleting both ANPs (should succeed)
		testNetworkConnectivity("https://www.baidu.com", true, "Testing connectivity to baidu.com after deleting both ANPs (should succeed)")
	})
})
