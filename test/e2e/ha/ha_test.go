...
var _ = framework.Describe("[group:ha]", func() {
	...
	framework.DisruptiveIt("ovn db should recover automatically from db file corruption", ginkgo.SpecTimeout(40*time.Second), func(ctx ginkgo.SpecContext) {
		...
		framework.WaitUntil(ctx, 30*time.Second, func(ctx context.Context) (bool, error) {
			pods, err := deployClient.GetAllPods(ctx, deploy)
			if err != nil {
				return false, err
			}
		...
		for _, node := range nodes {
			...
			pod, err := dsClient.GetPodOnNode(ctx, ds, node)
			framework.ExpectNoError(err)

			ginkgo.By("Ensuring db file " + db + " on node " + node + " is ok")
			stdout, stderr, err := framework.ExecShellInPod(ctx, f, pod.Namespace, pod.Name, checkCmd)
			...
			stdout, stderr, err = framework.ExecShellInPod(ctx, f, pod.Namespace, pod.Name, corruptCmd)
		...
		deployClient.SetScale(ctx, deploy.Name, replicas)
