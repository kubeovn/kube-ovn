...
var _ = framework.OrderedDescribe("[group:iptables-vpc-nat-gw]", func() {
	...
	framework.ConformanceIt("[6] node local EIP access via macvlan sub-interface", func() {
		...
		ginkgo.By("11. Verifying HOST_IP environment variable is set in NAT gateway pod")
		var hostIPEnvFound bool
				...
				for _, env := range container.Env {
					if env.Name == "HOST_IP" {
						...
						if env.ValueFrom != nil && env.ValueFrom.FieldRef != nil {
							framework.ExpectEqual(env.ValueFrom.FieldRef.FieldPath, "status.hostIP",
								"HOST_IP should reference status.hostIP")
						}
						break
					}
				}
		...
		framework.ExpectTrue(hostIPEnvFound, "HOST_IP environment variable should be defined in pod spec")

		ginkgo.By("12. Verifying HOST_IP value and route inside NAT gateway pod")
		hostIPValue := natGwPod.Status.HostIP
		framework.ExpectNotEmpty(hostIPValue, "Pod status.hostIP should not be empty")
		framework.Logf("NAT gateway pod HOST_IP: %s", hostIPValue)

		// Verify HOST_IP env is accessible inside the pod
		hostIPOut, hostIPErr, err := framework.ExecShellInContainer(f, framework.KubeOvnNamespace, vpcNatGwPodName, "vpc-nat-gw", "echo $HOST_IP")
		framework.ExpectNoError(err, "Failed to execute command in pod: stderr=%s", hostIPErr)
		podHostIP := strings.TrimSpace(hostIPOut)
		framework.ExpectEqual(podHostIP, hostIPValue, "HOST_IP env inside pod should match pod status.hostIP")

		// Verify route to HOST_IP exists inside the pod
		routeOut, routeErr, err := framework.ExecShellInContainer(f, framework.KubeOvnNamespace, vpcNatGwPodName, "vpc-nat-gw", "ip route show "+hostIPValue+"/32")
		framework.ExpectNoError(err, "Failed to show route in pod: stderr=%s", routeErr)
		framework.Logf("Route entry for HOST_IP/32: %s", routeOut)
		framework.ExpectNotEmpty(strings.TrimSpace(routeOut), "Route to HOST_IP/32 should exist inside NAT gateway pod")

		ginkgo.By("13. Test completed: Node local EIP access via macvlan sub-interface and pod-to-node route works correctly")
	})
})
