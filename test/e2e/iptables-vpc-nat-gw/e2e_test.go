...
func readNatGatewayScript() []byte {
	...
	if !ok {
		framework.Failf("failed to get caller information for nat-gateway.sh path resolution")
		return nil
	}

	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(thisFile), "..", "..", ".."))
	scriptPath := filepath.Join(repoRoot, "dist", "images", "vpcnatgateway", "nat-gateway.sh")

	content, err := os.ReadFile(scriptPath)
	if err != nil {
		framework.Failf("failed to read nat-gateway.sh from %s: %v", scriptPath, err)
		return nil
	}

	framework.Logf("Successfully read nat-gateway.sh from: %s", scriptPath)
	return content
}
...
var _ = framework.OrderedDescribe("[group:iptables-vpc-nat-gw]", func() {
	...
	framework.ConformanceIt("[1] change gateway image, custom annotations and custom namespace", func() {
		...
		newImage := os.Getenv("E2E_VPC_NAT_GW_IMAGE")
		if newImage == "" {
			newImage = oldImage
		}
		if newImage != oldImage {
			cm.Data["image"] = newImage
			cm, err = f.ClientSet.CoreV1().ConfigMaps(framework.KubeOvnNamespace).Update(context.Background(), cm, metav1.UpdateOptions{})
			framework.ExpectNoError(err)
			time.Sleep(3 * time.Second)
		} else {
			framework.Logf("skip image update for this case because E2E_VPC_NAT_GW_IMAGE is empty or unchanged")
		}
	...
	framework.ConformanceIt("[7] NAT Gateway scripts from host path", func() {
		...
		const expectedVolumeName = util.VpcNatGwScriptVolumeName
		const expectedMountPath = util.VpcNatGwScriptMountPath
		...
		gomega.Eventually(func() error {
			...
			for _, container := range pod.Spec.Containers {
				if container.Name == util.VpcNatGwContainerName {
