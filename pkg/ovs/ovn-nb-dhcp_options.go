...
func (c *OVNNbClient) updateDHCPOptions(dhcpOpt *ovnnb.DHCPOptions, fields ...any) error {
	...
	op, err := c.Database.WhereTable(dhcpOpt).Update(dhcpOpt, fields...)
	if err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) DeleteDHCPOptionsByUUIDs(uuidList ...string) error {
	...
	for _, uuid := range uuidList {
		...
		op, err := c.Database.WhereTable(dhcpOptions).Delete()
		if err != nil {
			klog.Error(err)
			return err
		}
...
func (c *OVNNbClient) DeleteDHCPOptions(lsName, protocol string) error {
	...
	op, err := c.Database.Table(&ovnnb.DHCPOptions{}).WhereCache(dhcpOptionsFilter(true, externalIDs)).Delete()
	if err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) DeleteDHCPOptionsForPort(portName string) error {
	...
	op, err := c.Database.Table(&ovnnb.DHCPOptions{}).WhereCache(dhcpOptionsFilter(true, map[string]string{
		PortKey: portName,
	})).Delete()
	if err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) ListDHCPOptions(needVendorFilter bool, externalIDs map[string]string) ([]ovnnb.DHCPOptions, error) {
	...
	if err := c.Database.Table(&ovnnb.DHCPOptions{}).Filter(ctx, dhcpOptionsFilter(needVendorFilter, externalIDs), &dhcpOptList); err != nil {
		klog.Error(err)
