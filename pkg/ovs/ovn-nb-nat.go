...
func (c *OVNNbClient) CreateNats(lrName string, nats ...*ovnnb.NAT) error {
	...
	createNatsOp, err := c.Database.Table(&ovnnb.NAT{}).CreateOps(models...)
	if err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) UpdateNat(nat *ovnnb.NAT, fields ...any) error {
	...
	op, err := c.Database.WhereTable(nat).Update(nat, fields...)
	if err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) GetNATByUUID(uuid string) (*ovnnb.NAT, error) {
	...
	if err := c.Database.Table(&ovnnb.NAT{}).Get(ctx, nat); err != nil {
		klog.Error(err)
		return nil, err
	}
...
func (c *OVNNbClient) listLogicalRouterNatByFilter(lrName string, filter func(route *ovnnb.NAT) bool) ([]*ovnnb.NAT, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), c.Timeout)
	defer cancel()
	if err := c.Database.Table(&ovnnb.NAT{}).Filter(ctx, predicate, &natList); err != nil {
		klog.Error(err)
		return nil, err
	}
