...
func (c *OVNNbClient) GetMeter(name string, ignoreNotFound bool) (*ovnnb.Meter, error) {
	...
	if err := c.Database.Table(&ovnnb.Meter{}).Get(ctx, meter); err != nil {
...
func (c *OVNNbClient) ListAllMeters() ([]*ovnnb.Meter, error) {
	...
	if err := c.Database.Table(&ovnnb.Meter{}).Filter(ctx, func(_ *ovnnb.Meter) bool {
		return true
	}, &meterList); err != nil {
...
func (c *OVNNbClient) updateMeterAndBand(meter *ovnnb.Meter, unit ovnnb.MeterUnit, rate, burst int) error {
	...
	if bandUUID != "" {
		...
		ctx, cancel := context.WithTimeout(context.Background(), c.Timeout)
		defer cancel()
		if err := c.Database.Table(&ovnnb.MeterBand{}).Get(ctx, band); err != nil {
			...
			bandUpdateOps, err = c.Database.WhereTable(band).Update(band, &band.Rate, &band.BurstSize)
			if err != nil {
	...
	if bandUUID == "" || len(bandUpdateOps) == 0 {
		...
		mutateOps, err := c.Database.WhereTable(meter).Mutate(meter, model.Mutation{
			Field:   &meter.Bands,
	...
	meter.Unit = unit
	updateMeterOps, err := c.Database.WhereTable(meter).Update(meter, &meter.Unit)
	if err != nil {
...
func (c *OVNNbClient) DeleteMeter(name string) error {
	...
	ops, err := c.Database.WhereTable(meter).Delete()
	if err != nil {
	...
	for _, bandUUID := range meter.Bands {
		band := &ovnnb.MeterBand{UUID: bandUUID}
		bandOps, err := c.Database.WhereTable(band).Delete()
		if err != nil {
