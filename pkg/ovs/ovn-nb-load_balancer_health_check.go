...
func (c *OVNNbClient) AddLoadBalancerHealthCheck(lbName, vipEndpoint string, externals map[string]string) error {
	...
	if err != nil {
		err := fmt.Errorf("failed to new lb health check: %w", err)
		klog.Error(err)
		return err
	}
...
func (c *OVNNbClient) newLoadBalancerHealthCheck(lbName, vipEndpoint string, externals map[string]string) (*ovnnb.LoadBalancerHealthCheck, error) {
	...
	if len(lbName) == 0 {
		err = errors.New("the lb name is required")
		klog.Error(err)
		return nil, err
	}
	if len(vipEndpoint) == 0 {
		err = errors.New("the vip endpoint is required")
		klog.Error(err)
		return nil, err
	}

	if exists, err = c.LoadBalancerHealthCheckExists(lbName, vipEndpoint); err != nil {
		err := fmt.Errorf("get lb health check %s: %w", vipEndpoint, err)
		klog.Error(err)
		return nil, err
	}
...
func (c *OVNNbClient) CreateLoadBalancerHealthCheck(lbName, vipEndpoint string, lbhc *ovnnb.LoadBalancerHealthCheck) error {
	...
	if createLbhcOp, err = c.Database.Table(&ovnnb.LoadBalancerHealthCheck{}).CreateOps(models...); err != nil {
		klog.Error(err)
	...
	if lbHcAddOp, err = c.LoadBalancerUpdateHealthCheckOp(lbName, lbhcUUIDs, ovsdb.MutateOperationInsert); err != nil {
		err = fmt.Errorf("generate operations for adding lbhc to lb %s: %w", lbName, err)
		klog.Error(err)
		return err
	}
	...
	if err = c.Transact("lbhc-add", ops); err != nil {
		err = fmt.Errorf("failed to create lb health check for lb %s vip %s: %w", lbName, vipEndpoint, err)
		klog.Error(err)
		return err
	}
...
func (c *OVNNbClient) UpdateLoadBalancerHealthCheck(lbhc *ovnnb.LoadBalancerHealthCheck, fields ...any) error {
	...
	if op, err = c.Database.WhereTable(lbhc).Update(lbhc, fields...); err != nil {
		klog.Error(err)
...
func (c *OVNNbClient) DeleteLoadBalancerHealthChecks(filter func(lb *ovnnb.LoadBalancerHealthCheck) bool) error {
	op, err := c.Database.Table(&ovnnb.LoadBalancerHealthCheck{}).WhereCache(
...
func (c *OVNNbClient) GetLoadBalancerHealthCheck(lbName, vipEndpoint string, ignoreNotFound bool) (*ovnnb.LoadBalancer, *ovnnb.LoadBalancerHealthCheck, error) {
	...
	if err = c.Database.Table(&ovnnb.LoadBalancerHealthCheck{}).Filter(ctx,
		...
		&healthCheckList); err != nil {
		err = fmt.Errorf("failed to list lb health check lb health check by vip %q: %w", vipEndpoint, err)
		klog.Error(err)
		return nil, nil, err
	}
...
func (c *OVNNbClient) ListLoadBalancerHealthChecks(filter func(lbhc *ovnnb.LoadBalancerHealthCheck) bool) ([]ovnnb.LoadBalancerHealthCheck, error) {
	...
	if err = c.Database.Table(&ovnnb.LoadBalancerHealthCheck{}).Filter(ctx,
		...
		&lbhcList); err != nil {
		return nil, fmt.Errorf("list lb health check: %w", err)
	}
...
func (c *OVNNbClient) DeleteLoadBalancerHealthCheckOp(lbName, vip string) ([]ovsdb.Operation, error) {
	...
	mutateOps, err := c.Database.WhereTable(lb).Mutate(lb, model.Mutation{
	...
	deleteOps, err := c.Database.WhereTable(lbhc).Delete()
	if err != nil {
		klog.Errorf("failed to generate operations for deleting lb health check: %v", err)
		return nil, err
	}
