...
func EnsureVswitchPort(ctx context.Context, provider table.Provider, config VswitchPortConfig) error {
	...
	if err := transactTable(ctx, portTable, "vswitch-port-ensure", operations); err != nil {
		return err
	}
	return waitForVswitchPort(ctx, provider, config)
}

func validateVswitchPortConfig(provider table.Provider, config VswitchPortConfig) error {
...
func waitForVswitchPort(ctx context.Context, provider table.Provider, config VswitchPortConfig) error {
	...
	}, "wait for OVS port %q cache update: %w", config.Port.Name)
	if err != nil {
		return err
	}
	if len(ports) != 1 {
	...
	_, err = waitForVswitchRows(ctx, provider, &vswitch.Bridge{}, func(row *vswitch.Bridge) bool {
		return row.Name == config.BridgeName && slices.Contains(row.Ports, portUUID)
	}, "wait for OVS bridge %q cache update: %w", config.BridgeName)
	return err
}

func mergeVswitchField(current map[string]string, desired *map[string]string) {
	if desired != nil && *desired != nil {
		*desired = mergeVswitchMap(current, *desired)
	}
}

func mergeVswitchInterfaceMaps(current, desired *vswitch.Interface) {
	mergeVswitchField(current.ExternalIDs, &desired.ExternalIDs)
	mergeVswitchField(current.Options, &desired.Options)
...
func findVswitchBridge(ctx context.Context, provider table.Provider, name string) (*vswitch.Bridge, error) {
	return findNamedRow(ctx, provider, &vswitch.Bridge{}, name, "bridge", true, func(row *vswitch.Bridge) string { return row.Name })
}

func findVswitchPort(ctx context.Context, provider table.Provider, name string) (*vswitch.Port, error) {
	return findNamedRow(ctx, provider, &vswitch.Port{}, name, "port", false, func(row *vswitch.Port) string { return row.Name })
}

func findVswitchInterface(ctx context.Context, provider table.Provider, name string) (*vswitch.Interface, error) {
...
func validateVswitchPortBridge(ctx context.Context, provider table.Provider, port *vswitch.Port, target *vswitch.Bridge) (bool, error) {
...
func DeleteVswitchPort(ctx context.Context, provider table.Provider, name string) error {
	...
	operations, err := appendVswitchPortDeletes(portTable, operations, ports, "delete OVS port %q: %w")
	if err != nil {
		return err
	}
	...
	return transactTable(ctx, portTable, "vswitch-port-delete", operations)
}
