	...
	for _, tt := range []struct {
		...
		t.Run(tt.kind, func(t *testing.T) {
			...
			if owner.kind != serviceLBOwnerKind || owner.namespace != svc.Namespace || owner.name != svc.Name || owner.uid != string(svc.UID) {
				t.Fatalf("forged controller owner reference changed ordinary Service owner to %#v", owner)
			}
			assertDeleteFilterIgnoresLB(t, svc, victim, "forged controller owner reference selected victim load balancer for deletion")
		})
	}
}
...
func TestRuleScopedLoadBalancerAttachments(t *testing.T) {
	...
	t.Run("nil enableLb follows controller default", func(t *testing.T) {
		fake, err := newFakeControllerWithOptions(t, &FakeControllerOptions{Subnets: []*kubeovnv1.Subnet{
			{Name: "subnet-nil", Spec: kubeovnv1.SubnetSpec{Vpc: util.DefaultVpc}},
...
func TestSubnetEnablesServiceLB(t *testing.T) {
	if !subnetEnablesServiceLB(&kubeovnv1.Subnet{}, true) {
		t.Fatal("nil EnableLb should follow default true")
	}
	if subnetEnablesServiceLB(&kubeovnv1.Subnet{}, false) {
		t.Fatal("nil EnableLb should follow default false")
	}
	if !subnetEnablesServiceLB(&kubeovnv1.Subnet{Spec: kubeovnv1.SubnetSpec{EnableLb: new(true)}}, false) {
		t.Fatal("explicit true should enable load balancers")
	}
	if subnetEnablesServiceLB(&kubeovnv1.Subnet{Spec: kubeovnv1.SubnetSpec{EnableLb: new(false)}}, true) {
		t.Fatal("explicit false should disable load balancers")
	}
}
...
func TestEnsureServiceScopedLBExternalTraffic(t *testing.T) {
	...
	gomock.InOrder(
		...
		fake.mockOvnClient.EXPECT().LogicalRouterUpdateLoadBalancers(util.DefaultVpc, ovsdb.MutateOperationInsert, lbName).Return(nil),
	)
...
func TestReconcileServiceScopedLoadBalancerAttachmentsClusterIP(t *testing.T) {
	fake, err := newFakeControllerWithOptions(t, &FakeControllerOptions{Subnets: []*kubeovnv1.Subnet{
		{Name: "subnet-a", Spec: kubeovnv1.SubnetSpec{Vpc: util.DefaultVpc, EnableLb: new(true)}},
		{Name: "subnet-b", Spec: kubeovnv1.SubnetSpec{Vpc: util.DefaultVpc, EnableLb: new(false)}},
	}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReconcileServiceScopedLoadBalancerAttachmentsLoadBalancerSeparatesTrafficScopes(t *testing.T) {
	fake, err := newFakeControllerWithOptions(t, &FakeControllerOptions{
		Vpcs:    []*kubeovnv1.Vpc{{Name: util.DefaultVpc}},
		Subnets: []*kubeovnv1.Subnet{{Name: "subnet-a", Spec: kubeovnv1.SubnetSpec{Vpc: util.DefaultVpc, EnableLb: new(true)}}},
	})
	...
	svc := &corev1.Service{
		Namespace: "default", Name: "web", UID: types.UID("svc-uid"),
		Spec: corev1.ServiceSpec{
			Type:      corev1.ServiceTypeLoadBalancer,
	...
	internal := serviceScopedLBNameForTrafficClass(svc, corev1.ProtocolTCP, serviceLBInternalTraffic)
	external := serviceScopedLBNameForTrafficClassAndFamily(svc, corev1.ProtocolTCP, serviceLBExternalTraffic, "ipv4")
	fake.mockOvnClient.EXPECT().LogicalSwitchUpdateLoadBalancers("subnet-a", ovsdb.MutateOperationInsert, internal, external).Return(nil)
	fake.mockOvnClient.EXPECT().LogicalRouterUpdateLoadBalancers(util.DefaultVpc, ovsdb.MutateOperationDelete, internal).Return(nil)
	fake.mockOvnClient.EXPECT().LogicalRouterUpdateLoadBalancers(util.DefaultVpc, ovsdb.MutateOperationInsert, external).Return(nil)

	if err := fake.fakeController.reconcileResourceScopedLoadBalancerAttachments(svc, util.DefaultVpc, "", internal, external); err != nil {
		t.Fatal(err)
	}
}
...
func TestServiceLBMigrationCandidates(t *testing.T) {
	...
	dual := &corev1.Service{
		Namespace: "default", Name: "web", UID: types.UID("uid-dual-migrate"),
...
func TestAttachServiceScopedLoadBalancersToRouter(t *testing.T) {
	fake, err := newFakeControllerWithOptions(t, &FakeControllerOptions{Vpcs: []*kubeovnv1.Vpc{
		{Name: util.DefaultVpc},
		{Name: "other-vpc"},
	}})
	...
	fake.mockOvnClient.EXPECT().LogicalRouterUpdateLoadBalancers(util.DefaultVpc, ovsdb.MutateOperationInsert, "svc-lb").Return(nil)
	fake.mockOvnClient.EXPECT().LogicalRouterUpdateLoadBalancers("other-vpc", ovsdb.MutateOperationDelete, "svc-lb").Return(nil)
	if err := fake.fakeController.attachServiceScopedLoadBalancersToRouter(util.DefaultVpc, "svc-lb"); err != nil {
		t.Fatal(err)
	}
}
...
func TestServiceScopedInternalLBFamilies(t *testing.T) {
	single := &corev1.Service{Spec: corev1.ServiceSpec{ClusterIP: "10.96.0.10", ClusterIPs: []string{"10.96.0.10"}}}
	if got := serviceScopedInternalLBFamilies(single); len(got) != 1 || got[0] != "" {
		t.Fatalf("single-stack families = %v, want [\"\"]", got)
	}

	dual := &corev1.Service{Spec: corev1.ServiceSpec{ClusterIP: "10.96.0.10", ClusterIPs: []string{"10.96.0.10", "fd00::10"}}}
	if got := serviceScopedInternalLBFamilies(dual); len(got) != 1 || got[0] != "" {
		t.Fatalf("non-template dual-stack families = %v, want [\"\"]", got)
	}

	trafficDistribution := corev1.ServiceTrafficDistributionPreferSameZone
	template := &corev1.Service{Spec: corev1.ServiceSpec{
		Type:                corev1.ServiceTypeClusterIP,
		ClusterIPs:          []string{"10.96.0.10", "fd00::10"},
		TrafficDistribution: &trafficDistribution,
	}}
	got := serviceScopedInternalLBFamilies(template)
	if len(got) != 2 {
		t.Fatalf("template dual-stack families = %v, want ipv4 and ipv6", got)
	}
	want := map[string]struct{}{"ipv4": {}, "ipv6": {}}
