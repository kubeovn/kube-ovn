...
func (c *Controller) enqueueEndpointSliceService(key string, services ...*v1.Service) {
	if key == "" || c.addOrUpdateEndpointSliceQueue == nil {
		return
	}

	priority := false
	for _, svc := range services {
		if svc != nil && serviceNeedsPriorityEndpointReconcile(svc) {
			priority = true
			break
		}
	}
	if !priority && len(services) == 0 && c.servicesLister != nil {
		namespace, name, err := cache.SplitMetaNamespaceKey(key)
		if err == nil {
			if svc, err := c.servicesLister.Services(namespace).Get(name); err == nil {
				priority = serviceNeedsPriorityEndpointReconcile(svc)
			}
		}
	}
	if priority && c.priorityEndpointSliceQueue != nil {
		c.priorityEndpointSliceQueue.Add(key)
		return
	}
...
// filterServiceEndpointSlices excludes stale selector-managed slices after a
// Service is repurposed as selectorless. The Kubernetes EndpointSlice
// controller eventually removes those slices, but they may remain in the
// informer while the Endpoints mirror already contains the desired targets.
func filterServiceEndpointSlices(svc *v1.Service, endpointSlices []*discoveryv1.EndpointSlice) []*discoveryv1.EndpointSlice {
	if serviceHasSelector(svc) {
		return endpointSlices
	}

	filtered := make([]*discoveryv1.EndpointSlice, 0, len(endpointSlices))
	for _, endpointSlice := range endpointSlices {
		if endpointSlice.Labels[discoveryv1.LabelManagedBy] == "endpointslice-controller.k8s.io" {
			continue
		}
...
func (c *Controller) endpointSliceServiceProfile(svc *v1.Service) (endpointSliceServiceProfile, error) {
	...
	if vip := svc.Annotations[util.SwitchLBRuleVipsAnnotation]; vip != "" {
		profile.addVIP(vip, serviceLBExternalTraffic, svc, true)
	} else if vips := svc.Annotations[util.RouterLBRuleVipsAnnotation]; vips != "" {
		enableHealthCheck := svc.Annotations[util.ServiceHealthCheck] == "true"
		for vip := range strings.SplitSeq(vips, ",") {
			if vip = strings.TrimSpace(vip); vip != "" {
				profile.addVIP(vip, serviceLBExternalTraffic, svc, enableHealthCheck)
			}
		}
	} else {
		for _, vip := range util.ServiceClusterIPs(*svc) {
			profile.addVIP(vip, serviceLBInternalTraffic, svc, false)
		}
	}
	if svc.Spec.Type == v1.ServiceTypeLoadBalancer {
		for _, ingress := range svc.Status.LoadBalancer.Ingress {
			if ingress.IP != "" {
				profile.addVIP(ingress.IP, serviceLBExternalTraffic, svc, false)
			}
		}
...
func (p *endpointSliceServiceProfile) addVIP(vip string, trafficClass serviceLBTrafficClass, svc *v1.Service, enableHealthCheck bool) {
	p.lbVips = append(p.lbVips, vip)
	p.trafficClasses[vip] = trafficClass
	if enableHealthCheck && util.CheckProtocol(vip) == kubeovnv1.ProtocolIPv4 && !serviceHealthChecksDisabled(svc) {
...
func (c *Controller) reconcileServiceEndpointVIP(reconcileCtx *endpointSliceReconcileContext, lbVip string, port v1.ServicePort) error {
	...
	if !isExternalVIP && serviceUsesScopedLB(svc) {
		families := serviceScopedInternalLBFamilies(svc)
		if len(families) > 1 || (len(families) == 1 && families[0] != "") {
			family := strings.ToLower(util.CheckProtocol(lbVip))
			state.lb = serviceScopedLBNameForTrafficClassAndFamily(svc, port.Protocol, serviceLBInternalTraffic, family)
		}
	}
...
func (c *Controller) serviceVIPIPPortMapping(reconcileCtx *endpointSliceReconcileContext, servicePort v1.ServicePort, serviceIP string, distributed bool, checkIP string) (IPPortMapping, error) {
	if distributed {
		...
		mapping = filterIPPortMappingByVIP(mapping, serviceIP)
		if !reconcileCtx.profile.ignoreHealthCheck {
	...
	return filterIPPortMappingByVIP(mapping, serviceIP), nil
}
...
func ipPortMappingAddress(backendIP string) string {
	if strings.HasPrefix(backendIP, "[") && strings.HasSuffix(backendIP, "]") {
		return backendIP[1 : len(backendIP)-1]
	}
	return backendIP
}

func filterIPPortMappingByVIP(mapping IPPortMapping, vip string) IPPortMapping {
	if len(mapping) == 0 {
		return mapping
	}
	protocol := util.CheckProtocol(vip)
	if protocol == "" {
		return mapping
	}
	filtered := make(IPPortMapping, len(mapping))
	for backendIP, value := range mapping {
		if util.CheckProtocol(ipPortMappingAddress(backendIP)) == protocol {
			filtered[backendIP] = value
		}
	}
	return filtered
}
