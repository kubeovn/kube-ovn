...
func (c *Controller) reconcileAllocateSubnets(pod *v1.Pod, needAllocatePodNets []*kubeovnNet) (*v1.Pod, error) {
	...
	// The Pod just received its addresses, which is what a NAT gateway Pod has to be
	// initialized with. Everything derived from those addresses, including pinning a
	// dynamically allocated one into spec.lanIp, is decided by the gateway itself.
	c.enqueueVpcNatGatewayInit(pod)
			...
			if c.config.EnableEipSnat {
				for ipStr := range strings.SplitSeq(podIP, ",") {
					if eip := pod.Annotations[util.EipAnnotation]; eip == "" {
						if err = c.deleteNats(c.config.ClusterRouter, ovnnb.NATTypeDNATAndSNAT, ipStr); err != nil {
					...
					if eip := pod.Annotations[util.SnatAnnotation]; eip == "" {
						if err = c.deleteNats(c.config.ClusterRouter, ovnnb.NATTypeSNAT, ipStr); err != nil {
							klog.Errorf("failed to delete nat rules: %v", err)
						}
					} else if util.CheckProtocol(eip) == util.CheckProtocol(ipStr) {
			...
			for _, address := range addresses {
				...
				if c.config.EnableEipSnat {
					if pod.Annotations[util.EipAnnotation] != "" {
						if err = c.deleteNat(c.config.ClusterRouter, ovnnb.NATTypeDNATAndSNAT, pod.Annotations[util.EipAnnotation], address.IP); err != nil {
							klog.Errorf("failed to delete nat rules: %v", err)
						}
					}
					if snatEip := pod.Annotations[util.SnatAnnotation]; snatEip != "" {
						if err = c.deleteNat(c.config.ClusterRouter, ovnnb.NATTypeSNAT, snatEip, address.IP); err != nil {
							klog.Errorf("failed to delete nat rules: %v", err)
						}
					}
				}
...
func needAllocateSubnets(pod *v1.Pod, nets []*kubeovnNet) []*kubeovnNet {
	...
	if job, ok := pod.Annotations[kubevirtv1.MigrationJobNameAnnotation]; ok &&
		pod.DeletionTimestamp.IsZero() && pod.Labels[kubevirtv1.MigrationJobLabel] != "" {
...
// enqueueVpcNatGatewayInit wakes the gateway initialization of a NAT gateway Pod that has just
// been given its addresses. Other Pods are ignored.
func (c *Controller) enqueueVpcNatGatewayInit(pod *v1.Pod) {
	if isVpcNatGw, vpcGwName := c.checkIsPodVpcNatGw(pod); isVpcNatGw {
