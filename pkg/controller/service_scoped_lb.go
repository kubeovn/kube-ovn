...
func subnetEnablesServiceLB(subnet *kubeovnv1.Subnet, defaultEnable bool) bool {
	if subnet.Spec.EnableLb != nil {
		return *subnet.Spec.EnableLb
	}
...
func serviceScopedInternalLBFamilies(svc *v1.Service) []string {
...
func (c *Controller) attachServiceScopedLoadBalancersToRouter(vpcName string, lbNames ...string) error {
	if vpcName == "" || len(lbNames) == 0 {
		return nil
	}
	if err := c.OVNNbClient.LogicalRouterUpdateLoadBalancers(vpcName, ovsdb.MutateOperationInsert, lbNames...); err != nil {
		return fmt.Errorf("attach service-scoped load balancers to logical router %s: %w", vpcName, err)
	}
	vpcs, err := c.vpcsLister.List(labels.Everything())
	if err != nil {
		return fmt.Errorf("list logical routers for service-scoped load balancer attachment: %w", err)
	}
