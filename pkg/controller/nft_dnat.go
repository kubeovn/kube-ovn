...
// It removes the vmap element and the per-identity chain atomically, on every running gateway
// instance (see createNftDnatMapInPod).
...
// applyShareDnatIdentity programs one share DNAT identity on every running gateway instance: the
// rule's EIP, and — for a rule generated by the nftable LB service feature, which records the
// Service's ClusterIP on itself — that internal VIP with the same backends, plus the in-pod state
// the VIP needs.
//
// The ClusterIP identity shares the EIP identity's backends because both belong to one Service
// port: the siblings getShareBackends aggregates are exactly that Service's backends.
func (c *Controller) applyShareDnatIdentity(rule *kubeovnv1.IptablesDnatRule, gwName, protocol, v4ip, externalPort string, backends []string) error {
	if err := c.createNftDnatMapInPod(gwName, protocol, v4ip, externalPort, backends, rule.Spec.SessionAffinity, rule.Spec.SessionAffinityTimeoutSeconds); err != nil {
		return err
	}
	clusterIP := nftableLbSvcClusterIPFromRule(rule)
	if clusterIP == "" {
		return nil
	}
	if err := c.createNftDnatMapInPod(gwName, protocol, clusterIP, externalPort, backends, rule.Spec.SessionAffinity, rule.Spec.SessionAffinityTimeoutSeconds); err != nil {
		return err
	}
	return c.applyShareDnatVipState(gwName, protocol, externalPort, clusterIP, v4ip)
}

// applyShareDnatVipState ensures the in-pod state shared by the two VIPs of one Service port: the
// ClusterIP is held on lo so the gateway owns the internal VIP locally, and each VIP gets a
// hairpin SNAT rule so VPC-originated traffic that gets DNAT'd back into the VPC returns to the
// instance holding the conntrack. Both operations are idempotent, so they are re-applied whenever
// an identity is programmed, including the gateway redo path after a Pod restart.
//
// The gateway VIP lock keeps this from interleaving with the teardown of another identity of the
// same gateway (see natGwVipKeyMutex): the lo address is shared by all identities of a Service, so
// a concurrent release could otherwise undo an ensure and leave the VIP half programmed.
func (c *Controller) applyShareDnatVipState(gwName, protocol, externalPort, clusterIP, eipIP string) error {
	...
	if err = c.execNatGwRulesInPods(gwPods, natGwVipAddrAdd, []string{clusterIP}); err != nil {
		return err
	}
	return c.execNatGwRulesInPods(gwPods, natGwVipHairpinAdd, []string{
		fmt.Sprintf("%s,%s,%s", eipIP, externalPort, protocol),
		fmt.Sprintf("%s,%s,%s", clusterIP, externalPort, protocol),
	})
}

// deleteShareDnatIdentity removes one share DNAT identity from every running gateway instance: the
// rule's EIP, and — for a generated rule — its ClusterIP identity and the VIP state that no
// remaining rule of the gateway needs any more.
func (c *Controller) deleteShareDnatIdentity(rule *kubeovnv1.IptablesDnatRule, gwName, protocol, v4ip, externalPort string) error {
	...
	if err := c.deleteNftDnatMapInPod(gwName, protocol, clusterIP, externalPort); err != nil {
		return err
	}
	return c.deleteShareDnatVipState(gwName, protocol, externalPort, clusterIP, v4ip)
}

// deleteShareDnatVipState removes the VIP state of a torn down share DNAT identity: the hairpin
// SNAT rules of both VIPs, and — once no live rule of the gateway records it any more — the
// ClusterIP held on lo. One gateway serves many Services, so the address is only released when
// the last rule referencing it is gone. A rule that is already terminating counts as gone: it
// will never be programmed again.
func (c *Controller) deleteShareDnatVipState(gwName, protocol, externalPort, clusterIP, eipIP string) error {
	c.natGwVipKeyMutex.LockKey(gwName)
	defer func() { _ = c.natGwVipKeyMutex.UnlockKey(gwName) }()
	...
	if err = c.execNatGwRulesInPods(gwPods, natGwVipHairpinDel, []string{
		fmt.Sprintf("%s,%s,%s", eipIP, externalPort, protocol),
		fmt.Sprintf("%s,%s,%s", clusterIP, externalPort, protocol),
	}); err != nil {
		return err
	}
	clusterIPInUse, err := c.shareDnatClusterIPInUse(gwName, clusterIP)
	if err != nil {
		return err
	}
	if clusterIPInUse {
	...
	return c.execNatGwRulesInPods(gwPods, natGwVipAddrDel, []string{clusterIP})
}

// shareDnatClusterIPInUse reports whether any live rule of the given gateway still records the
// given internal VIP.
func (c *Controller) shareDnatClusterIPInUse(gwName, clusterIP string) (bool, error) {
	rules, err := c.iptablesDnatRulesLister.List(labels.SelectorFromSet(labels.Set{util.VpcNatGatewayNameLabel: gwName}))
	if err != nil {
		return false, err
	}
	for _, rule := range rules {
		if rule.DeletionTimestamp.IsZero() && nftableLbSvcClusterIPFromRule(rule) == clusterIP {
			return true, nil
		}
	}
	return false, nil
...
// desiredNatGwVipRouteVips returns the VIPs the given gateway has to route, keyed by policy route
// match. Only rules generated by the nftable LB service feature contribute: the feature owns these
// routes, and a hand-managed share DNAT rule keeps working the way it did before.
func (c *Controller) desiredNatGwVipRouteVips(gwName string) (map[string]string, error) {
	rules, err := c.iptablesDnatRulesLister.List(labels.SelectorFromSet(labels.Set{util.VpcNatGatewayNameLabel: gwName}))
	if err != nil {
		return nil, err
	}
	...
	for _, rule := range rules {
		...
		if !rule.DeletionTimestamp.IsZero() || rule.Spec.Type != kubeovnv1.DnatRuleTypeShare ||
			nftableLbSvcOwnerKey(rule) == "" {
			continue
		}
		if clusterIP := nftableLbSvcClusterIPFromRule(rule); util.CheckProtocol(clusterIP) == kubeovnv1.ProtocolIPv4 {
			vips[natGwVipRouteMatch(clusterIP)] = clusterIP
		}
		ip, resolved := eipIPs[rule.Spec.EIP]
		if !resolved {
			eip, err := c.GetEip(rule.Spec.EIP)
			if err != nil {
				klog.Errorf("nat gw %s: failed to resolve eip %s of dnat %s for its vip route: %v", gwName, rule.Spec.EIP, rule.Name, err)
			} else {
				ip = eip.Status.IP
			}
...
// natGwVipRouteNextHops returns the sorted LAN addresses of the gateway's ready instances, which
// are the next hops of its VIP routes. Every instance holds the VIP identity, so any of them can
// serve the traffic.
func (c *Controller) natGwVipRouteNextHops(gw *kubeovnv1.VpcNatGateway) ([]string, error) {
	pods, err := c.listNatGwPods(gw)
	if err != nil {
		return nil, err
	}
	nextHopByNode, err := getNatGwNextHops(gw, pods)
	if err != nil {
		return nil, err
	}
...
// syncNatGwVipRoutes reconciles the VPC policy routes that send traffic destined for a share DNAT
...
func (c *Controller) syncNatGwVipRoutes(gwName string) error {
	...
	desired, err := c.desiredNatGwVipRouteVips(gwName)
	if err != nil {
		return err
	}
	nextHops, err := c.natGwVipRouteNextHops(gw)
	if err != nil {
		return err
	}
	if len(nextHops) == 0 {
		// Without a reachable instance there is no next hop to route to: remove the routes and
		// let the traffic take the path it took before this feature existed, instead of
		// programming a reroute policy without next hops (which OVN rejects).
...
func (c *Controller) cleanupShareDnatInPod(key string, rule *kubeovnv1.IptablesDnatRule, gwName, protocol, v4ip, externalPort string) error {
	remainingBackends, affinity, affinityTimeout, err := c.getShareBackends(gwName, rule.Spec.EIP, externalPort, protocol, rule.Name)
	if err != nil {
	...
	if clusterIP := nftableLbSvcClusterIPFromRule(rule); clusterIP != "" {
		if err = c.createNftDnatMapInPod(gwName, protocol, clusterIP, externalPort, remainingBackends, affinity, affinityTimeout); err != nil {
			return fmt.Errorf("failed to rebuild nft dnat map for %s: %w", key, err)
		}
	}
	return nil
