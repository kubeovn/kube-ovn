...
func TestControllerProviderReadsAssociatedRows(t *testing.T) {
...
func TestControllerProviderWrites(t *testing.T) {
...
func TestControllerProviderFieldUpdates(t *testing.T) {
	backend := newTableBackend(
		&ovnnb.LoadBalancer{
			UUID:    "lb-1",
			Name:    "lb-1",
...
func TestControllerProviderCollectionUpdates(t *testing.T) {
	backend := newTableBackend(
...
func TestControllerProviderRelationUpdates(t *testing.T) {
	backend := newTableBackend(
...
func TestControllerProviderNBGlobalUpdates(t *testing.T) {
...
func TestControllerProviderParentReferenceDeletes(t *testing.T) {
	backend := newTableBackend(
		&ovnnb.LogicalRouter{
			UUID:     "lr-1",
			Name:     "lr-1",
...
func TestControllerProviderNatCreate(t *testing.T) {
...
func TestControllerProviderNatReconcile(t *testing.T) {
	backend := newTableBackend(&ovnnb.LogicalRouter{UUID: "lr-1", Name: "lr-1"})
	controller := &Controller{OVNNbTables: table.NewDatabase(backend, time.Second, table.RetryPolicy{})}
...
func TestControllerProviderPeerAndPatchPorts(t *testing.T) {
...
func TestControllerProviderLogicalSwitchRepairsMissingPatchPorts(t *testing.T) {
	backend := newTableBackend(
		&ovnnb.LogicalSwitch{UUID: "ls-1", Name: "switch-1"},
		&ovnnb.LogicalRouter{UUID: "lr-1", Name: "router-1"},
	)
...
func TestControllerProviderLogicalPatchPortRepairsPartialTopology(t *testing.T) {
	tests := []struct {
		name        string
	...
	}{
		{
			...
			existing: []model.Model{&ovnnb.LogicalSwitchPort{
				...
				ExternalIDs: map[string]string{ovs.LogicalSwitchKey: "switch-1", "vendor": util.CniTypeName},
			}},
			wantCreate:  1,
			wantLSPUUID: "lsp-1",
			wantLabels:  []string{"mutate:LogicalSwitch", "create:LogicalRouterPort", "mutate:LogicalRouter"},
		},
		{
			name:        "logical router port exists",
			existing:    []model.Model{&ovnnb.LogicalRouterPort{UUID: "lrp-1", Name: "lrp-1", Networks: []string{"10.0.0.1/24"}, ExternalIDs: map[string]string{"lr": "router-1", "vendor": util.CniTypeName}}},
			wantCreate:  1,
			wantLRPUUID: "lrp-1",
			wantLabels:  []string{"create:LogicalSwitchPort", "mutate:LogicalSwitch", "mutate:LogicalRouter"},
		},
		{
			name: "both ports exist detached",
			existing: []model.Model{
				&ovnnb.LogicalSwitchPort{UUID: "lsp-1", Name: "lsp-1", Type: "router", Options: map[string]string{"router-port": "lrp-1"}, ExternalIDs: map[string]string{ovs.LogicalSwitchKey: "switch-1", "vendor": util.CniTypeName}},
				&ovnnb.LogicalRouterPort{UUID: "lrp-1", Name: "lrp-1", Networks: []string{"10.0.0.1/24"}, ExternalIDs: map[string]string{"lr": "router-1", "vendor": util.CniTypeName}},
			},
			wantLSPUUID: "lsp-1",
			wantLRPUUID: "lrp-1",
			wantLabels:  []string{"mutate:LogicalSwitch", "mutate:LogicalRouter"},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(t, 2, backend.mutateCalls)
			require.Equal(t, 1, backend.transactCalls)
			require.Equal(t, test.wantLabels, backend.transactionLabels[0])
...
func TestControllerProviderLogicalSwitchPortMovesBetweenSwitches(t *testing.T) {
	backend := newTableBackend(
	...
	require.Equal(t, 1, backend.transactCalls)
	require.Equal(t, []string{"mutate:LogicalSwitch", "create:LogicalSwitchPort", "mutate:LogicalSwitch"}, backend.transactionLabels[0])
}

func TestControllerProviderLogicalPatchPortsMoveBetweenParents(t *testing.T) {
	backend := newTableBackend(
	...
	require.Len(t, backend.updated, 2)
	updatedLSP, ok := backend.updated[0].(*ovnnb.LogicalSwitchPort)
	require.True(t, ok)
	require.Equal(t, "switch-new", updatedLSP.ExternalIDs[ovs.LogicalSwitchKey])
	updatedLRP, ok := backend.updated[1].(*ovnnb.LogicalRouterPort)
	require.True(t, ok)
	require.Equal(t, "router-new", updatedLRP.ExternalIDs["lr"])
	...
	require.Equal(t, []string{
		"mutate:LogicalSwitch", "update:LogicalSwitchPort", "mutate:LogicalSwitch",
		"mutate:LogicalRouter", "update:LogicalRouterPort", "mutate:LogicalRouter",
	}, backend.transactionLabels[0])
}

func TestControllerProviderPolicyReconcile(t *testing.T) {
	backend := newTableBackend(&ovnnb.LogicalRouter{UUID: "lr-1", Name: "lr-1"})
	controller := &Controller{OVNNbTables: table.NewDatabase(backend, time.Second, table.RetryPolicy{})}
...
func TestControllerProviderRouterPortRA(t *testing.T) {
...
func TestControllerProviderLogicalSwitchAndDHCP(t *testing.T) {
...
func TestControllerProviderLogicalSwitchACL(t *testing.T) {
...
func TestControllerProviderPortDeletes(t *testing.T) {
	backend := newTableBackend(
...
func TestControllerProviderHAChassisGroup(t *testing.T) {
	backend := newTableBackend(
...
func TestControllerProviderMeterDelete(t *testing.T) {
	backend := newTableBackend(
...
func TestControllerProviderStaticRoutes(t *testing.T) {
...
func TestControllerProviderPortCreates(t *testing.T) {
	backend := newTableBackend(
		&ovnnb.LogicalSwitch{UUID: "ls-1", Name: "ls-1"},
		&ovnnb.LogicalRouter{UUID: "lr-1", Name: "lr-1"},
	)
...
func TestControllerProviderLoadBalancerOperations(t *testing.T) {
	backend := newTableBackend(
		&ovnnb.LoadBalancer{
			UUID:           "lb-1",
			Name:           "lb-1",
...
func TestControllerProviderACLHelpers(t *testing.T) {
	pgName := "node-pg"
...
func (b *tableBackend) Create(rows ...model.Model) ([]ovsdb.Operation, error) {
	...
	comment := "create:" + rowType.Name()
	return []ovsdb.Operation{{Op: ovsdb.OperationComment, Comment: &comment}}, nil
}
...
func (b *tableBackend) Transact(_ context.Context, operations ...ovsdb.Operation) ([]ovsdb.OperationResult, error) {
	...
	for _, operation := range operations {
		if operation.Comment != nil {
			labels = append(labels, *operation.Comment)
		}
	}
	b.transactionLabels = append(b.transactionLabels, labels)
...
func (c tableConditional) Mutate(row model.Model, mutations ...model.Mutation) ([]ovsdb.Operation, error) {
	...
	comment := "mutate:" + rowType.Name()
	return []ovsdb.Operation{{Op: ovsdb.OperationComment, Comment: &comment}}, nil
}

func (c tableConditional) Update(row model.Model, _ ...any) ([]ovsdb.Operation, error) {
	c.backend.updateCalls++
	c.backend.updated = append(c.backend.updated, row)
	rowType := reflect.TypeOf(row)
	if rowType.Kind() == reflect.Pointer {
		rowType = rowType.Elem()
	}
	comment := "update:" + rowType.Name()
	return []ovsdb.Operation{{Op: ovsdb.OperationComment, Comment: &comment}}, nil
}
