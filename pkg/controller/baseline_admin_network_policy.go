...
type BanpDelta struct {
	oldBanp v1alpha1.BaselineAdminNetworkPolicy
	newBanp v1alpha1.BaselineAdminNetworkPolicy
}
...
func (c *Controller) processNextUpdateBanpWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		var key BanpDelta
		var ok bool
		if key, ok = obj.(BanpDelta); !ok {
			c.updateBanpQueue.Forget(obj)
			utilruntime.HandleError(fmt.Errorf("expected BanpDelta in workqueue but got %#v", obj))
			return nil
		}
		if err := c.handleUpdateBanp(key); err != nil {
			c.updateBanpQueue.AddRateLimited(key)
			return fmt.Errorf("error syncing banp %s: %v, requeuing", key.newBanp.Name, err)
		}
		c.updateBanpQueue.Forget(obj)
		return nil
	}(obj)
...
func (c *Controller) handleAddBanp(key string) (err error) {
	listBanps, err := c.banpsLister.List(labels.Everything())
	if err != nil {
		klog.Error(err)
		return err
	}
	if len(listBanps) > 1 {
		err := fmt.Errorf("Onle one banp can be created, there's %v in cluster now", len(listBanps))
		klog.Error(err)
		return err
	}
	...
	pgName := strings.ReplaceAll(fmt.Sprintf("%s", banpName), "-", ".")
	...
	// create ingress acl
	for idx, banpr := range banp.Spec.Ingress {
		...
		ingressAsV4Name := strings.ReplaceAll(fmt.Sprintf("%s.%s.%s", pgName, banpr.Name, kubeovnv1.ProtocolIPv4), "-", ".")
		ingressAsV6Name := strings.ReplaceAll(fmt.Sprintf("%s.%s.%s", pgName, banpr.Name, kubeovnv1.ProtocolIPv6), "-", ".")
		...
		aclPriority := util.BanpAclMaxPriority - idx
		aclAction := convertAction("", banpr.Action)
		if len(v4Addrs) != 0 {
			ops, err := c.OVNNbClient.UpdateAnpRuleACLOps(pgName, ingressAsV4Name, kubeovnv1.ProtocolIPv4, aclPriority, aclAction, *banpr.Ports, true)
			if err != nil {
		...
		if len(v6Addrs) != 0 {
			ops, err := c.OVNNbClient.UpdateAnpRuleACLOps(pgName, ingressAsV6Name, kubeovnv1.ProtocolIPv6, aclPriority, aclAction, *banpr.Ports, true)
			if err != nil {
	...
	// create egress acl
	for idx, banpr := range banp.Spec.Egress {
		// A single address set must contain addresses of the same type and the name must be unique within table, so IPv4 and IPv6 address set should be different
		egressAsV4Name := strings.ReplaceAll(fmt.Sprintf("%s.%s.%s", pgName, banpr.Name, kubeovnv1.ProtocolIPv4), "-", ".")
		egressAsV6Name := strings.ReplaceAll(fmt.Sprintf("%s.%s.%s", pgName, banpr.Name, kubeovnv1.ProtocolIPv6), "-", ".")
		...
		aclAction := convertAction("", banpr.Action)
		if len(v4Addrs) != 0 {
			ops, err := c.OVNNbClient.UpdateAnpRuleACLOps(pgName, egressAsV4Name, kubeovnv1.ProtocolIPv4, aclPriority, aclAction, *banpr.Ports, false)
			if err != nil {
		...
		if len(v6Addrs) != 0 {
			ops, err := c.OVNNbClient.UpdateAnpRuleACLOps(pgName, egressAsV6Name, kubeovnv1.ProtocolIPv6, aclPriority, aclAction, *banpr.Ports, false)
			if err != nil {
...
func (c *Controller) handleUpdateBanp(changed BanpDelta) error {
	// Only handle updates that do not affect acls.
	c.banpKeyMutex.LockKey(changed.newBanp.Name)
	defer func() { _ = c.banpKeyMutex.UnlockKey(changed.newBanp.Name) }()

	curBanp := changed.oldBanp
	desiredBanp := changed.newBanp
	...
	// The port-group for anp should be updated
	if !reflect.DeepEqual(curBanp.Spec.Subject, desiredBanp.Spec.Subject) {
		...
		if ok, err := c.OVNNbClient.PortGroupExists(pgName); !ok || err != nil {
			klog.Error("port-group for banp %s does not exist when update banp", desiredBanp.Name)
			return err
		}
	...
	for index, rule := range desiredBanp.Spec.Ingress {
		oldRule := curBanp.Spec.Ingress[index]
		if !reflect.DeepEqual(oldRule.From, rule.From) {
			if err := c.setAddrSetForAnpRule(banpName, pgName, rule.Name, rule.From, []v1alpha1.AdminNetworkPolicyEgressPeer{}, true, true); err != nil {
	...
	for index, rule := range desiredBanp.Spec.Egress {
		oldRule := curBanp.Spec.Egress[index]
		if !reflect.DeepEqual(oldRule.To, rule.To) {
			if err := c.setAddrSetForAnpRule(banpName, pgName, rule.Name, []v1alpha1.AdminNetworkPolicyIngressPeer{}, rule.To, false, true); err != nil {
