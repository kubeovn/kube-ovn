...
func (c *Controller) removeExternalGateway() error {
	...
	for _, orino := range nodes {
		// considering that this node will soon be updated, may it not be necessary to be deep-coped?
		// need further confirmed.
		// mainly considering the gap time between modify and result returned from apiserver.
		no := orino.DeepCopy()
