...
func (c *Controller) handleAddNode(key string) error {
	...
	// There is only one nodeAddr since k8s/node does not support dualstack
	nodeAddr := getNodeInternalIP(node)
	if util.CheckProtocol(nodeAddr) == kubeovnv1.ProtocolIPv4 {
	...
	if util.CheckProtocol(nodeAddr) == kubeovnv1.ProtocolIPv6 {
...
func (c *Controller) handleDeleteNode(key string) error {
	...
	if len(addresses) > 0 {
		...
		if len(addresses) == 2 {
			if err := c.ovnClient.DeleteStaticRouteByNextHop(addresses[1].Ip); err != nil {
				return err
			}
		}
	}
