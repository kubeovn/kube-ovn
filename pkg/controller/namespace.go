...
func (c *Controller) enqueueUpdateNamespace(oldObj, newObj interface{}) {
	...
	if !reflect.DeepEqual(oldNs.Labels, newNs.Labels) {
		...
		for _, subnet := range subnets {
			changed := false
			for _, nsSelector := range subnet.Spec.NamespaceSelector {
			...
			if slices.Contains(subnet.Spec.Namespaces, newNs.Name) {
				expectSubnets = append(expectSubnets, subnet.Name)
			}
		}
		// when ns delete labels which match subnet.spec.namespaceSelector, the annotation logical_switch for the ns will be updated
...
func (c *Controller) handleAddNamespace(key string) error {
	...
	for _, s := range subnets {
		...
		for _, nsSelector := range s.Spec.NamespaceSelector {
			matchSelector, err := mergeSelector(nsSelector)
			if err != nil {
				klog.Errorf("failed to merge selector, %v", err)
				return err
