...
func (c *Controller) handleAddOrUpdateBgpEdgeRouterAdvertisement(key string) error {
	...
	subnetName := advertisement.Spec.Subnets
	if subnetName == "" {
	...
	deploy, err := c.berDeploymentsLister.Deployments(advertisement.Namespace).Get(advertisement.Spec.BerName)
	if err != nil {
		klog.Error(err)
		return err
	}

	advertisement.Status.Workload.APIVersion = deploy.APIVersion
	advertisement.Status.Workload.Kind = deploy.Kind
	advertisement.Status.Workload.Name = deploy.Name
	advertisement.Status.Workload.Nodes = nil
	nodeNexthopIPv4 := make(map[string]string, int(advertisement.Spec.Replicas))
	nodeNexthopIPv6 := make(map[string]string, int(advertisement.Spec.Replicas))
...
func (c *Controller) reconcileBgpEdgeRouterWorkload(advertisement *kubeovnv1.BgpEdgeRouterAdvertisement, vpc *kubeovnv1.Vpc) (string, set.Set[string], set.Set[string], *appsv1.Deployment, error) {
	...
	if currentDeploy, err := c.berDeploymentsLister.Deployments(advertisement.Namespace).Get(deploy.Name); err != nil {
		if !k8serrors.IsNotFound(err) {
