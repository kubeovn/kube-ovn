...
func ValidateSubnet(subnet kubeovnv1.Subnet) error {
	...
	if subnet.Spec.U2OFeatures.DisableInternalUnderlayDirectRouting {
		if !subnet.Spec.U2OInterconnection {
			return errors.New("u2oFeatures.disableInternalUnderlayDirectRouting can only be enabled when u2OInterconnection is true")
		}
		if subnet.Spec.Vlan == "" {
			return errors.New("u2oFeatures.disableInternalUnderlayDirectRouting can only be enabled on underlay subnets (vlan must be set)")
		}
	}
