...
func ParseFlags() (*Configuration, error) {
	...
	for range 3 {
		pod, err := config.KubeClient.CoreV1().Pods(config.PodNamespace).Get(context.Background(), podName, metav1.GetOptions{})
		if err != nil {
			klog.Errorf("failed to get self pod %s/%s: %v", config.PodNamespace, podName, err)
			return nil, err
		}

		if config.LabelSelector == "" {
			var dsName string
			for owner := range slices.Values(pod.OwnerReferences) {
				if owner.APIVersion == appsv1.SchemeGroupVersion.String() && owner.Kind == util.KindDaemonSet {
					dsName = owner.Name
					break
				}
			}
			ds, err := config.KubeClient.AppsV1().DaemonSets(config.PodNamespace).Get(context.Background(), dsName, metav1.GetOptions{})
			if err != nil {
				klog.Errorf("failed to get ds %s/%s: %v", config.PodNamespace, dsName, err)
				return nil, err
			}
			config.LabelSelector = labels.Set(ds.Spec.Selector.MatchLabels).String()
		}
		...
		if len(pod.Status.ContainerStatuses) != 0 && pod.Status.ContainerStatuses[0].Ready {
			util.LogFatalAndExit(nil, "failed to get IPs of Pod %s/%s: podIPs is empty while the container is ready", config.PodNamespace, podName)
		}
	...
	if len(config.PodProtocols) == 0 {
		util.LogFatalAndExit(nil, "failed to get IPs of Pod %s/%s after 3 attempts", config.PodNamespace, podName)
	}
