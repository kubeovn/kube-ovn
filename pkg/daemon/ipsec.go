...
func configureOVSWithIPSecKeys(p pkiFiles) error {
	cmd := exec.Command("ovs-vsctl", "--retry", "-t", "60", "set", "Open_vSwitch", ".", "other_config:certificate="+p.certificatePath, "other_config:private_key="+p.privateKeyPath, "other_config:ca_cert="+p.caCertPath)
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func getOVSIPSecKeys() (pkiFiles, error) {
	...
	if err != nil {
		if strings.Index(string(certificate), "no key \"certificate\"") >= 0 {
			return pkiFiles{}, nil
		}
	...
	if err != nil {
		if strings.Index(string(certificate), "no key \"private_key\"") >= 0 {
			return pkiFiles{}, nil
		}
	...
	if err != nil {
		if strings.Index(string(certificate), "no key \"ca_cert\"") >= 0 {
			return pkiFiles{}, nil
		}
...
func linkCACertToIPSecDir(ca []byte) error {
	...
	if err := os.MkdirAll(ipsecCADir, 0755); err != nil {
	...
	for i, cert := range certificates {
		...
		if err := os.WriteFile(filepath, cert, 0644); err != nil {
			return fmt.Errorf("failed to write certificate %s: %w", filename, err)
		}

		klog.Info("Wrote CA certificate %s", filename)
	}

	klog.V(3).Info("Total certificates processed: %d", len(certificates))
	return nil
}
