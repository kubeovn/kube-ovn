...
func (csh cniServerHandler) handleAdd(req *restful.Request, resp *restful.Response) {
	...
	if strings.HasSuffix(podRequest.Provider, util.OvnProvider) && subnet != "" {
		...
		if err = ovs.ConfigInterfaceMirror(csh.Config.EnableMirror, pod.Annotations[fmt.Sprintf(util.MirrorControlAnnotationTemplate, podRequest.Provider)], ifaceID, csh.Config.VswitchTables); err != nil {
