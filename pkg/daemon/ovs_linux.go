...
// configProviderVlanInterfaces creates OVS internal ports for VLAN interfaces and transfers IPs
func (c *Controller) configProviderVlanInterfaces(vlanInterfaceMap map[string]int, brName string) error {
	for vlanIfName, vlanID := range vlanInterfaceMap {
		...
		// Transfer addresses from kernel VLAN interface to OVS internal port
...
// transferVlanAddrsToInternalPort transfers addresses and routes from a VLAN interface to an OVS internal port
// This is a simplified version of transferAddrsAndRoutes that doesn't try to set bridge MAC addresses
func (c *Controller) transferVlanAddrsToInternalPort(srcName, dstName string) error {
	...
	// Bring up the destination interface
...
func (c *Controller) removeProviderVlanInterface(internalPortName, brName string, vlanID int) error {
	...
	// Get the base interface name (e.g., eth0 from br-eth0-vlan10)
	parts := strings.Split(internalPortName, "-vlan")
	if len(parts) != 2 {
		return fmt.Errorf("invalid VLAN internal port name format: %s", internalPortName)
	}
	baseInterfaceName := strings.TrimPrefix(parts[0], "br-") // Remove br- prefix to get eth0

	// Check if the OVS internal port exists
	...
	// Get addresses and routes from the internal port
	...
	// Remove the internal port from OVS bridge
	...
	// If there are addresses to restore, recreate the kernel VLAN interface
	if len(addrs) > 0 || len(routes) > 0 {
		...
		// Check if kernel VLAN interface already exists
		...
		// Get the newly created or existing VLAN interface
		...
		// Restore addresses to kernel VLAN interface
		for _, addr := range addrs {
			if addr.IP.IsLinkLocalUnicast() {
		...
		// Restore routes to kernel VLAN interface
