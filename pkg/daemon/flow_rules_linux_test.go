...
func TestUnderlayServiceLocalFlows(t *testing.T) {
	...
	wantLocal := "cookie=0x1000,priority=10000,in_port=7,tcp,nw_dst=172.19.0.100,tp_dst=80 actions=mod_dl_dst:00:00:00:01:00:01,in_port"
	if got[0] != wantPhy {
		t.Fatalf("physical steal flow = %q, want %q", got[0], wantPhy)
	}
	if got[1] != wantLocal {
		t.Fatalf("localnet bounce flow = %q, want %q", got[1], wantLocal)
	}
}
