...
func TestBandwidthRateToMbps(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "fractional decimal SI quantity", rate: intstr.FromString("1.5G"), want: 1500},
		{name: "fractional binary SI quantity", rate: intstr.FromString(".5Gi"), want: 537},
		...
		{name: "whitespace", rate: intstr.FromString(" 1024 "), wantErr: formatErr},
		{name: "decimal without suffix", rate: intstr.FromString("1.5"), wantErr: formatErr},
		{name: "milli suffix", rate: intstr.FromString("1m"), wantErr: formatErr},
		{name: "kilo suffix", rate: intstr.FromString("500k"), wantErr: formatErr},
		{name: "kibi suffix", rate: intstr.FromString("1Ki"), wantErr: formatErr},
		{name: "tera suffix", rate: intstr.FromString("1T"), wantErr: formatErr},
		{name: "decimal exponent", rate: intstr.FromString("1e6"), wantErr: formatErr},
		{name: "invalid suffix", rate: intstr.FromString("1GB"), wantErr: formatErr},
		{name: "invalid quantity", rate: intstr.FromString("fast"), wantErr: formatErr},
