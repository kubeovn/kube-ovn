...
var _ = Describe("[IPAM]", func() {
	...
	Describe("[IPAM]", func() {
		It("subnet not found", func() {
			im := NewIPAM()
			Expect(im.GetStaticAddress("pod", "eth0", "", "", subnetName, true)).Error().Should(MatchError(ErrNoAvailable))
			Expect(im.GetSubnetV4Mask(subnetName)).Error().Should(MatchError(ErrNoAvailable))
		})
		...
		Context("[IPv4]", func() {
			It("invalid subnet", func() {
				...
				Expect(err).Should(MatchError(ErrInvalidCIDR))
				err = im.AddOrUpdateSubnet(subnetName, "1.1.256.1/24", v4Gw, nil)
				Expect(err).Should(MatchError(ErrInvalidCIDR))
				Expect(im.Subnets[subnetName]).Should(BeNil())
			})
			...
			It("normal subnet", func() {
				...
				Expect(im.Subnets[subnetName]).ShouldNot(BeNil())
				Expect(im.GetSubnetV4Mask(subnetName)).Should(Equal("16"))
				...
				Expect(err).Should(MatchError(ErrConflict))
	...
	Describe("[Subnet]", func() {
		Context("[IPv4]", func() {
			It("init subnet", func() {
				subnet, err := NewSubnet(subnetName, ipv4CIDR, ipv4ExcludeIPs)
				Expect(err).ShouldNot(HaveOccurred())
				...
				Expect(subnet.V4FreeIPList).To(Equal(
					IPRangeList{
						&IPRange{Start: "10.16.0.2", End: "10.16.0.3"},
						&IPRange{Start: "10.16.0.5", End: "10.16.0.9"},
						&IPRange{Start: "10.16.0.24", End: "10.16.255.254"},
					}))
			})
			...
			It("static allocation", func() {
				...
				Expect(subnet.V4IPToPod).To(HaveKeyWithValue(IP("10.16.0.2"), "pod1.ns"))
				Expect(subnet.V4IPToPod).To(HaveKeyWithValue(IP("10.16.0.3"), "pod2.ns"))
				Expect(subnet.V4IPToPod).To(HaveKeyWithValue(IP("10.16.0.20"), "pod3.ns"))
				Expect(subnet.V4NicToIP).To(HaveKeyWithValue("pod1.ns", IP("10.16.0.2")))
				Expect(subnet.V4NicToIP).To(HaveKeyWithValue("pod2.ns", IP("10.16.0.3")))
				Expect(subnet.V4NicToIP).To(HaveKeyWithValue("pod3.ns", IP("10.16.0.20")))
				...
				Expect(err).Should(MatchError(ErrOutOfRange))
			...
			It("random allocation", func() {
				subnet, err := NewSubnet(subnetName, "10.16.0.0/30", nil)
				Expect(err).ShouldNot(HaveOccurred())

				ip1, _, _, err := subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip1).To(Equal(IP("10.16.0.1")))
				ip1, _, _, err = subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip1).To(Equal(IP("10.16.0.1")))
				...
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip2).To(Equal(IP("10.16.0.2")))

				_, _, _, err = subnet.GetRandomAddress("pod3.ns", "pod3.ns", "", nil, true)
				Expect(err).Should(MatchError(ErrNoAvailable))
				Expect(subnet.V4FreeIPList).To(BeEmpty())

				Expect(subnet.V4IPToPod).To(HaveKeyWithValue(IP("10.16.0.1"), "pod1.ns"))
				Expect(subnet.V4IPToPod).To(HaveKeyWithValue(IP("10.16.0.2"), "pod2.ns"))
				Expect(subnet.V4NicToIP).To(HaveKeyWithValue("pod1.ns", IP("10.16.0.1")))
				Expect(subnet.V4NicToIP).To(HaveKeyWithValue("pod2.ns", IP("10.16.0.2")))

				subnet.ReleaseAddress("pod1.ns")
				subnet.ReleaseAddress("pod2.ns")
				Expect(subnet.V4FreeIPList).To(Equal(IPRangeList{}))
				Expect(subnet.V4ReleasedIPList).To(Equal(IPRangeList{&IPRange{Start: "10.16.0.1", End: "10.16.0.2"}}))
				Expect(subnet.V4IPToPod).To(BeEmpty())
				Expect(subnet.V4NicToIP).To(BeEmpty())
			})
		})
		...
		Context("[IPv6]", func() {
			It("init subnet", func() {
				subnet, err := NewSubnet(subnetName, ipv6CIDR, ipv6ExcludeIPs)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(subnet.Name).To(Equal(subnetName))
				...
				Expect(subnet.V6FreeIPList).To(Equal(
					IPRangeList{
						&IPRange{Start: "fd00::2", End: "fd00::3"},
						&IPRange{Start: "fd00::5", End: "fd00::9"},
						&IPRange{Start: "fd00::18", End: "fd00::fffe"},
					}))
			})
			...
			It("static allocation", func() {
				...
				Expect(subnet.V6IPToPod).To(HaveKeyWithValue(IP("fd00::2"), "pod1.ns"))
				Expect(subnet.V6IPToPod).To(HaveKeyWithValue(IP("fd00::3"), "pod2.ns"))
				Expect(subnet.V6IPToPod).To(HaveKeyWithValue(IP("fd00::14"), "pod3.ns"))
				Expect(subnet.V6NicToIP).To(HaveKeyWithValue("pod1.ns", IP("fd00::2")))
				Expect(subnet.V6NicToIP).To(HaveKeyWithValue("pod2.ns", IP("fd00::3")))
				Expect(subnet.V6NicToIP).To(HaveKeyWithValue("pod3.ns", IP("fd00::14")))
			...
			It("random allocation", func() {
				subnet, err := NewSubnet(subnetName, "fd00::/126", nil)
				Expect(err).ShouldNot(HaveOccurred())

				_, ip1, _, err := subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip1).To(Equal(IP("fd00::1")))
				_, ip1, _, err = subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip1).To(Equal(IP("fd00::1")))
				...
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ip2).To(Equal(IP("fd00::2")))

				_, _, _, err = subnet.GetRandomAddress("pod3.ns", "pod3.ns", "", nil, true)
				Expect(err).Should(MatchError(ErrNoAvailable))
				Expect(subnet.V6FreeIPList).To(BeEmpty())

				Expect(subnet.V6IPToPod).To(HaveKeyWithValue(IP("fd00::1"), "pod1.ns"))
				Expect(subnet.V6IPToPod).To(HaveKeyWithValue(IP("fd00::2"), "pod2.ns"))
				Expect(subnet.V6NicToIP).To(HaveKeyWithValue("pod1.ns", IP("fd00::1")))
				Expect(subnet.V6NicToIP).To(HaveKeyWithValue("pod2.ns", IP("fd00::2")))

				subnet.ReleaseAddress("pod1.ns")
				subnet.ReleaseAddress("pod2.ns")
				Expect(subnet.V6FreeIPList).To(Equal(IPRangeList{}))
				Expect(subnet.V6ReleasedIPList).To(Equal(IPRangeList{&IPRange{Start: "fd00::1", End: "fd00::2"}}))
				Expect(subnet.V6IPToPod).To(BeEmpty())
				Expect(subnet.V6NicToIP).To(BeEmpty())
			})
		})
		...
		Context("[DualStack]", func() {
			It("init subnet", func() {
				subnet, err := NewSubnet(subnetName, dualCIDR, dualExcludeIPs)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(subnet.Name).To(Equal(subnetName))
				Expect(subnet.V4ReservedIPList).To(HaveLen(len(ipv4ExcludeIPs) - 1))
				Expect(subnet.V4FreeIPList).To(HaveLen(3))
			...
			It("random allocation", func() {
				subnet, err := NewSubnet(subnetName, "10.16.0.0/30,fd00::/126", nil)
				Expect(err).ShouldNot(HaveOccurred())

				ipv4, ipv6, _, err := subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ipv4).To(Equal(IP("10.16.0.1")))
				Expect(ipv6).To(Equal(IP("fd00::1")))
				ipv4, ipv6, _, err = subnet.GetRandomAddress("pod1.ns", "pod1.ns", "", nil, true)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ipv4).To(Equal(IP("10.16.0.1")))
				Expect(ipv6).To(Equal(IP("fd00::1")))
				...
				Expect(err).ShouldNot(HaveOccurred())
				Expect(ipv4).To(Equal(IP("10.16.0.2")))
				Expect(ipv6).To(Equal(IP("fd00::2")))
