...
// OvnFipApplyConfiguration represents a declarative configuration of the OvnFip type for use
// with apply.
...
// OvnFip constructs a declarative configuration of the OvnFip type for use with
// apply.
...
// ExtractOvnFipFrom extracts the applied configuration owned by fieldManager from
// ovnFip for the specified subresource. Pass an empty string for subresource to extract
// the main resource. Common subresources include "status", "scale", etc.
// ovnFip must be a unmodified OvnFip API object that was retrieved from the Kubernetes API.
// ExtractOvnFipFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractOvnFipFrom(ovnFip *kubeovnv1.OvnFip, fieldManager string, subresource string) (*OvnFipApplyConfiguration, error) {
	b := &OvnFipApplyConfiguration{}
	err := managedfields.ExtractInto(ovnFip, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.OvnFip"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(ovnFip.Name)
...
// ExtractOvnFip extracts the applied configuration owned by fieldManager from
// ovnFip. If no managedFields are found in ovnFip for fieldManager, a
// OvnFipApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// ovnFip must be a unmodified OvnFip API object that was retrieved from the Kubernetes API.
// ExtractOvnFip provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractOvnFip(ovnFip *kubeovnv1.OvnFip, fieldManager string) (*OvnFipApplyConfiguration, error) {
	return ExtractOvnFipFrom(ovnFip, fieldManager, "")
}

// ExtractOvnFipStatus extracts the applied configuration owned by fieldManager from
// ovnFip for the status subresource.
func ExtractOvnFipStatus(ovnFip *kubeovnv1.OvnFip, fieldManager string) (*OvnFipApplyConfiguration, error) {
	return ExtractOvnFipFrom(ovnFip, fieldManager, "status")
}
...
func (b *OvnFipApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
