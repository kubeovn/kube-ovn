...
// IptablesEIPApplyConfiguration represents a declarative configuration of the IptablesEIP type for use
// with apply.
...
func IptablesEIP(name string) *IptablesEIPApplyConfiguration {
	b := &IptablesEIPApplyConfiguration{}
	b.WithName(name)
	b.WithKind("IptablesEIP")
	b.WithAPIVersion("kubeovn.io/v1")
	return b
}

// ExtractIptablesEIPFrom extracts the applied configuration owned by fieldManager from
// iptablesEIP for the specified subresource. Pass an empty string for subresource to extract
// the main resource. Common subresources include "status", "scale", etc.
// iptablesEIP must be a unmodified IptablesEIP API object that was retrieved from the Kubernetes API.
// ExtractIptablesEIPFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractIptablesEIPFrom(iptablesEIP *kubeovnv1.IptablesEIP, fieldManager string, subresource string) (*IptablesEIPApplyConfiguration, error) {
	b := &IptablesEIPApplyConfiguration{}
	err := managedfields.ExtractInto(iptablesEIP, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.IptablesEIP"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(iptablesEIP.Name)
...
// ExtractIptablesEIP extracts the applied configuration owned by fieldManager from
// iptablesEIP. If no managedFields are found in iptablesEIP for fieldManager, a
// IptablesEIPApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// iptablesEIP must be a unmodified IptablesEIP API object that was retrieved from the Kubernetes API.
// ExtractIptablesEIP provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractIptablesEIP(iptablesEIP *kubeovnv1.IptablesEIP, fieldManager string) (*IptablesEIPApplyConfiguration, error) {
	return ExtractIptablesEIPFrom(iptablesEIP, fieldManager, "")
}

// ExtractIptablesEIPStatus extracts the applied configuration owned by fieldManager from
// iptablesEIP for the status subresource.
func ExtractIptablesEIPStatus(iptablesEIP *kubeovnv1.IptablesEIP, fieldManager string) (*IptablesEIPApplyConfiguration, error) {
	return ExtractIptablesEIPFrom(iptablesEIP, fieldManager, "status")
}
...
func (b *IptablesEIPApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
