...
func ExtractIPFrom(iP *kubeovnv1.IP, fieldManager string, subresource string) (*IPApplyConfiguration, error) {
	b := &IPApplyConfiguration{}
	err := managedfields.ExtractInto(iP, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.IP"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(iP.Name)
...
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractIP(iP *kubeovnv1.IP, fieldManager string) (*IPApplyConfiguration, error) {
