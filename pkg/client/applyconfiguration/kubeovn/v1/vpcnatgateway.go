...
// VpcNatGatewayApplyConfiguration represents a declarative configuration of the VpcNatGateway type for use
// with apply.
...
func VpcNatGateway(name string) *VpcNatGatewayApplyConfiguration {
	b := &VpcNatGatewayApplyConfiguration{}
	b.WithName(name)
	b.WithKind("VpcNatGateway")
	b.WithAPIVersion("kubeovn.io/v1")
	return b
}

// ExtractVpcNatGatewayFrom extracts the applied configuration owned by fieldManager from
// vpcNatGateway for the specified subresource. Pass an empty string for subresource to extract
// the main resource. Common subresources include "status", "scale", etc.
// vpcNatGateway must be a unmodified VpcNatGateway API object that was retrieved from the Kubernetes API.
// ExtractVpcNatGatewayFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractVpcNatGatewayFrom(vpcNatGateway *kubeovnv1.VpcNatGateway, fieldManager string, subresource string) (*VpcNatGatewayApplyConfiguration, error) {
	b := &VpcNatGatewayApplyConfiguration{}
	err := managedfields.ExtractInto(vpcNatGateway, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.VpcNatGateway"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(vpcNatGateway.Name)
...
// ExtractVpcNatGateway extracts the applied configuration owned by fieldManager from
// vpcNatGateway. If no managedFields are found in vpcNatGateway for fieldManager, a
// VpcNatGatewayApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// vpcNatGateway must be a unmodified VpcNatGateway API object that was retrieved from the Kubernetes API.
// ExtractVpcNatGateway provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractVpcNatGateway(vpcNatGateway *kubeovnv1.VpcNatGateway, fieldManager string) (*VpcNatGatewayApplyConfiguration, error) {
	return ExtractVpcNatGatewayFrom(vpcNatGateway, fieldManager, "")
}

// ExtractVpcNatGatewayStatus extracts the applied configuration owned by fieldManager from
// vpcNatGateway for the status subresource.
func ExtractVpcNatGatewayStatus(vpcNatGateway *kubeovnv1.VpcNatGateway, fieldManager string) (*VpcNatGatewayApplyConfiguration, error) {
	return ExtractVpcNatGatewayFrom(vpcNatGateway, fieldManager, "status")
}
...
func (b *VpcNatGatewayApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
