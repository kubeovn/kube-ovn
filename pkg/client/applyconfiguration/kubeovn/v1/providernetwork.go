...
// ProviderNetworkApplyConfiguration represents a declarative configuration of the ProviderNetwork type for use
// with apply.
...
// ProviderNetwork constructs a declarative configuration of the ProviderNetwork type for use with
// apply.
func ProviderNetwork(name string) *ProviderNetworkApplyConfiguration {
	b := &ProviderNetworkApplyConfiguration{}
	b.WithName(name)
	b.WithKind("ProviderNetwork")
	b.WithAPIVersion("kubeovn.io/v1")
	return b
}

// ExtractProviderNetworkFrom extracts the applied configuration owned by fieldManager from
// providerNetwork for the specified subresource. Pass an empty string for subresource to extract
// the main resource. Common subresources include "status", "scale", etc.
// providerNetwork must be a unmodified ProviderNetwork API object that was retrieved from the Kubernetes API.
// ExtractProviderNetworkFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractProviderNetworkFrom(providerNetwork *kubeovnv1.ProviderNetwork, fieldManager string, subresource string) (*ProviderNetworkApplyConfiguration, error) {
	b := &ProviderNetworkApplyConfiguration{}
	err := managedfields.ExtractInto(providerNetwork, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.ProviderNetwork"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(providerNetwork.Name)
...
// ExtractProviderNetwork extracts the applied configuration owned by fieldManager from
// providerNetwork. If no managedFields are found in providerNetwork for fieldManager, a
// ProviderNetworkApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// providerNetwork must be a unmodified ProviderNetwork API object that was retrieved from the Kubernetes API.
// ExtractProviderNetwork provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractProviderNetwork(providerNetwork *kubeovnv1.ProviderNetwork, fieldManager string) (*ProviderNetworkApplyConfiguration, error) {
	return ExtractProviderNetworkFrom(providerNetwork, fieldManager, "")
}

// ExtractProviderNetworkStatus extracts the applied configuration owned by fieldManager from
// providerNetwork for the status subresource.
func ExtractProviderNetworkStatus(providerNetwork *kubeovnv1.ProviderNetwork, fieldManager string) (*ProviderNetworkApplyConfiguration, error) {
	return ExtractProviderNetworkFrom(providerNetwork, fieldManager, "status")
}
...
func (b *ProviderNetworkApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
