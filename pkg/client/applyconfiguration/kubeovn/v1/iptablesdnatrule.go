...
// IptablesDnatRuleApplyConfiguration represents a declarative configuration of the IptablesDnatRule type for use
// with apply.
...
// IptablesDnatRule constructs a declarative configuration of the IptablesDnatRule type for use with
// apply.
func IptablesDnatRule(name string) *IptablesDnatRuleApplyConfiguration {
	b := &IptablesDnatRuleApplyConfiguration{}
	b.WithName(name)
	b.WithKind("IptablesDnatRule")
	b.WithAPIVersion("kubeovn.io/v1")
	return b
}

// ExtractIptablesDnatRuleFrom extracts the applied configuration owned by fieldManager from
// iptablesDnatRule for the specified subresource. Pass an empty string for subresource to extract
// the main resource. Common subresources include "status", "scale", etc.
// iptablesDnatRule must be a unmodified IptablesDnatRule API object that was retrieved from the Kubernetes API.
// ExtractIptablesDnatRuleFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractIptablesDnatRuleFrom(iptablesDnatRule *kubeovnv1.IptablesDnatRule, fieldManager string, subresource string) (*IptablesDnatRuleApplyConfiguration, error) {
	b := &IptablesDnatRuleApplyConfiguration{}
	err := managedfields.ExtractInto(iptablesDnatRule, internal.Parser().Type("com.github.kubeovn.kube-ovn.pkg.apis.kubeovn.v1.IptablesDnatRule"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(iptablesDnatRule.Name)
...
// ExtractIptablesDnatRule extracts the applied configuration owned by fieldManager from
// iptablesDnatRule. If no managedFields are found in iptablesDnatRule for fieldManager, a
// IptablesDnatRuleApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// iptablesDnatRule must be a unmodified IptablesDnatRule API object that was retrieved from the Kubernetes API.
// ExtractIptablesDnatRule provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractIptablesDnatRule(iptablesDnatRule *kubeovnv1.IptablesDnatRule, fieldManager string) (*IptablesDnatRuleApplyConfiguration, error) {
	return ExtractIptablesDnatRuleFrom(iptablesDnatRule, fieldManager, "")
}

// ExtractIptablesDnatRuleStatus extracts the applied configuration owned by fieldManager from
// iptablesDnatRule for the status subresource.
func ExtractIptablesDnatRuleStatus(iptablesDnatRule *kubeovnv1.IptablesDnatRule, fieldManager string) (*IptablesDnatRuleApplyConfiguration, error) {
	return ExtractIptablesDnatRuleFrom(iptablesDnatRule, fieldManager, "status")
}
...
func (b *IptablesDnatRuleApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
