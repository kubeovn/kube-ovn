...
type BgpConfLister interface {
	...
	List(selector labels.Selector) (ret []*kubeovnv1.BgpConf, err error)
	// Get retrieves the BgpConf from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kubeovnv1.BgpConf, error)
	BgpConfListerExpansion
}

// bgpConfLister implements the BgpConfLister interface.
type bgpConfLister struct {
	listers.ResourceIndexer[*kubeovnv1.BgpConf]
}

// NewBgpConfLister returns a new BgpConfLister.
func NewBgpConfLister(indexer cache.Indexer) BgpConfLister {
	return &bgpConfLister{listers.New[*kubeovnv1.BgpConf](indexer, kubeovnv1.Resource("bgpconf"))}
}
