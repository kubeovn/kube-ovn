...
ENABLE_SSL_VPN_GW=${ENABLE_SSL_VPN_GW:-true}
...
kind: CustomResourceDefinition
metadata:
  name: vpc-ssl-vpn-gateways.kubeovn.io
spec:
  group: kubeovn.io
  names:
    plural: vpc-ssl-vpn-gateways
    singular: vpc-ssl-vpn-gateway
    shortNames:
      - vpc-ssl-vpn-gw
    kind: VpcSslVpnGateway
    listKind: VpcSslVpnGatewayList
  scope: Cluster
  versions:
    - additionalPrinterColumns:
...
rules:
  - apiGroups:
    ...
    resources:
      ...
      - vpc-ssl-vpn-gateways
      - vpc-ssl-vpn-gateways/status
      - subnets
      - subnets/status
      - ips
      - vips
...
data:
  ...
  ssl-vpn-image: "registry.cn-hangzhou.aliyuncs.com/bobz/openvpn:v0.0.4"
---
kind: ConfigMap
apiVersion: v1
metadata:
...
apiVersion: v1
metadata:
  name: ovn-vpc-ssl-vpn-gw-config
  namespace: kube-system
data:
  enable-vpc-ssl-vpn-gw: "$ENABLE_SSL_VPN_GW"
---
kind: Deployment
apiVersion: apps/v1
metadata:
