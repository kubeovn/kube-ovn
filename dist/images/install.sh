...
metadata:
  annotations:
    controller-gen.kubebuilder.io/version: v0.20.1
  name: vpc-wireguard-peers.kubeovn.io
spec:
  group: kubeovn.io
  names:
    kind: VpcWireGuardPeer
    listKind: VpcWireGuardPeerList
    plural: vpc-wireguard-peers
    shortNames:
    - vpc-wg-peer
    singular: vpc-wireguard-peer
  scope: Cluster
  versions:
  - additionalPrinterColumns:
    - jsonPath: .spec.wireGuard
      name: WireGuard
      type: string
    - jsonPath: .status.clientIP
      name: ClientIP
      type: string
    - jsonPath: .status.ready
      name: Ready
      ...
      openAPIV3Schema:
        properties:
          ...
          spec:
            properties:
              clientIP:
                description: Optional static client IP in the WireGuard clientSubnet.
                  Empty means allocate.
                type: string
              generateKey:
                description: Generate a client keypair into a Secret. Mutually exclusive
                  with publicKey.
                type: boolean
              persistentKeepalive:
                description: PersistentKeepalive interval in seconds. 0 disables it.
                format: int32
                maximum: 65535
                minimum: 0
                type: integer
              presharedKeySecretRef:
                description: Optional preshared key secret.
                properties:
                  key:
                ...
                x-kubernetes-map-type: atomic
              publicKey:
                description: Client public key. Required when generateKey is false.
                type: string
              wireGuard:
                description: VpcWireGuard name this peer belongs to. Immutable after
                  creation.
                type: string
                x-kubernetes-validations:
                - message: wireGuard is immutable
                  rule: self == oldSelf
            type: object
          status:
            properties:
              clientIP:
                description: Allocated or requested client tunnel IP.
                type: string
              conditions:
                description: Conditions represent the latest state of the object.
                items:
                  description: Condition describes the state of an object at a certain
                    point.
                  properties:
                ...
                type: array
              configSecret:
                description: Secret containing wg-quick.conf when generateKey is true.
                type: string
              endpoint:
                description: Public endpoint copied from the gateway.
                type: string
              publicKey:
                description: Client public key actually programmed on the server.
                type: string
              ready:
                description: Ready state of the peer.
                type: boolean
              serverPublicKey:
                description: Server public key copied from the gateway.
                type: string
            type: object
        type: object
    served: true
...
metadata:
  annotations:
    controller-gen.kubebuilder.io/version: v0.20.1
  name: vpc-wireguards.kubeovn.io
spec:
  group: kubeovn.io
  names:
    kind: VpcWireGuard
    listKind: VpcWireGuardList
    plural: vpc-wireguards
    shortNames:
    - vpc-wg
    singular: vpc-wireguard
  scope: Cluster
  versions:
  - additionalPrinterColumns:
    - jsonPath: .spec.vpc
      ...
      openAPIV3Schema:
        properties:
          ...
          spec:
            properties:
              affinity:
                ...
                type: object
              allowedIPs:
                description: CIDRs advertised to clients as AllowedIPs. Empty means
                  all CIDRs of the VPC.
                items:
                  type: string
                type: array
              clientSubnet:
                description: Overlay subnet used as the client IP pool. Immutable
                  after creation.
                type: string
                x-kubernetes-validations:
                - message: clientSubnet is immutable
                  rule: self == oldSelf
              exposure:
                description: How the server is reached from the internet.
                properties:
                  eip:
                    description: IptablesEIP name. Required for DNAT and FIP. Optional
                      static hint for DualNIC.
                    type: string
                  externalSubnets:
                    description: External/underlay subnets for DualNIC (first entry
                      is used).
                    items:
                      type: string
                    type: array
                  natGateway:
                    description: VpcNatGateway name that owns the EIP. Required for
                      DNAT and FIP.
                    type: string
                  type:
                    ...
                    type: string
                    x-kubernetes-validations:
                    - message: exposure type is immutable
                      rule: self == oldSelf
                type: object
              generateServerKey:
                description: Generate a server keypair into a Secret. Mutually exclusive
                  with publicKey.
                type: boolean
              lanIp:
                description: Optional static LAN IP on spec.subnet.
                type: string
              listenPort:
                default: 51820
                description: UDP listen port. Defaults to 51820.
                format: int32
                maximum: 65535
                minimum: 1
                type: integer
              ...
              namespace:
                ...
                type: string
              privateKeySecretRef:
                description: Secret holding the existing server private key when generateServerKey
                  is false.
                properties:
                  key:
                    description: The key of the secret to select from.  Must be a
                ...
                x-kubernetes-map-type: atomic
              publicKey:
                description: Existing server public key when generateServerKey is
                  false.
                type: string
              selector:
                description: Pod selector for the WireGuard server.
                items:
                  type: string
                type: array
              subnet:
                description: Overlay subnet for the WireGuard server pod LAN NIC.
              ...
              tolerations:
                ...
                type: array
              vpc:
                description: VPC name. Immutable after creation.
                type: string
                x-kubernetes-validations:
          ...
          status:
            properties:
              clientCIDR:
                description: Client CIDR from clientSubnet.
                type: string
              conditions:
                description: Conditions represent the latest state of the object.
                items:
                ...
                type: array
              endpoint:
                description: Public endpoint host:port clients should use.
                type: string
              lanIp:
                description: LAN IP of the server pod.
                type: string
              publicKey:
                description: Server public key.
                type: string
              ready:
                description: Ready state of the WireGuard server.
                type: boolean
              serverKeySecret:
                description: Secret storing the generated server private key.
                type: string
              serverTunnelIP:
                description: Tunnel address assigned to wg0 from clientSubnet.
                type: string
            type: object
        type: object
        x-kubernetes-validations:
...
rules:
  - apiGroups:
    ...
    resources:
      ...
      - vpc-wireguards
      - vpc-wireguards/status
      - vpc-wireguard-peers
      - vpc-wireguard-peers/status
      - vpc-egress-gateways
