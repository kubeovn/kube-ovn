  ...
  case $type in
    icmp)
      ...
      kubectl exec "$OVN_SB_POD" -n $KUBE_OVN_NS -c ovn-central -- ovn-trace --ct=new --ct=new --ct=new --ct=new "$ls" \
        "inport == \"$podName.$namespace\" && ip.ttl == 255 && icmp$icmp_type && eth.src == $mac && ip$af.src == $podIP && eth.dst == $dstMac && ip$af.dst == $dst && ct.new"
      ;;
    tcp|udp)
      tcp_flags=""
      if [ "$type" = "tcp" ]; then
      ...
      set -x
      kubectl exec "$OVN_SB_POD" -n $KUBE_OVN_NS -c ovn-central -- ovn-trace --ct=new --ct=new --ct=new --ct=new "$ls" \
        "inport == \"$podName.$namespace\" && ip.ttl == 255 && eth.src == $mac && ip$af.src == $podIP && eth.dst == $dstMac && ip$af.dst == $dst && $type.src == 30000 && $type.dst == $4 $tcp_flags && ct.new"
      ;;
    *)
