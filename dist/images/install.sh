...
kind: CustomResourceDefinition
metadata:
  name: bgp-edge-routers.kubeovn.io
spec:
  group: kubeovn.io
  names:
    plural: bgp-edge-routers
    singular: bgp-edge-router
    shortNames:
      - bgp-er
      - ber
    kind: BgpEdgeRouter
    listKind: BgpEdgeRouterList
  scope: Namespaced
  versions:
    - additionalPrinterColumns:
        - jsonPath: .spec.vpc
            ...
            spec:
              ...
              x-kubernetes-validations:
                ...
                - rule: "!has(self.externalIPs) || size(self.externalIPs) == 0 || size(self.externalIPs) >= self.replicas"
                  ...
                  fieldPath: ".externalIPs"
                - rule: "size(self.policies) != 0 || size(self.selectors) != 0"
                  message: 'Each BGP Edeg Router MUST have at least one policy or selector'
              properties:
                replicas:
                  type: integer
                  format: int32
                ...
                bgp:
                  ...
                  properties:
                    ...
                    routerId:
                      type: string
                      # The routerId can be an IPv4 or IPv6 address, but we are not enforcing it here
                      # routerId could be nullable, so that we can use environment value to set it
                      # refer pkg/speaker/config.go#L186
                      anyOf:
                        - format: ipv4
                        - format: ipv6
                        - pattern: '^$'
...
kind: CustomResourceDefinition
metadata:
  name: bgp-edge-router-advertisements.kubeovn.io
spec:
  group: kubeovn.io
  names:
    plural: bgp-edge-router-advertisements
    singular: bgp-edge-router-advertisement
    shortNames:
      - ber-ad
    kind: BgpEdgeRouterAdvertisement
    listKind: BgpEdgeRouterAdvertisementList
  scope: Namespaced
  versions:
    - additionalPrinterColumns:
        - jsonPath: .spec.subnet
...
kind: CustomResourceDefinition
metadata:
  name: gobgp-configs.kubeovn.io
spec:
  group: kubeovn.io
  names:
    plural: gobgp-configs
    singular: gobgp-config
    shortNames:
      - bgp-config
    kind: GobgpConfig
    listKind: GobgpConfigList
  scope: Namespaced
  versions:
    - name: v1
      served: true
      ...
      additionalPrinterColumns:
        - name: bgp-edge-router
          type: string
          jsonPath: .spec.bgpEdgeRouterInfo.name
        - name: neighbor-address
          type: string
          jsonPath: .spec.neighbors[*].address
        - name: toReceive-mode
          type: string
          jsonPath: .spec.neighbors[*].toReceive.allowed.mode
        - name: toReceive-prefixes
          type: string
          jsonPath: .spec.neighbors[*].toReceive.allowed.prefixes
        - name: toAdvertise-mode
          type: string
          jsonPath: .spec.neighbors[*].toAdvertise.allowed.mode
        - name: toAdvertise-prefixes
          type: string
          jsonPath: .spec.neighbors[*].toAdvertise.allowed.prefixes
        - name: READY
          type: boolean
          jsonPath: .status.ready
        - name: AGE
                      ...
                      toAdvertise:
                        ...
                        properties:
                          allowed:
                            ...
                            x-kubernetes-validations:
                              - rule: "self.mode != 'filtered' || (has(self.prefixes) && size(self.prefixes) > 0)"
                                message: "prefixes must be set and non-empty when mode is 'filtered'"
                      toReceive:
                        type: object
                        required:
                          - allowed
...
rules:
  - apiGroups:
    ...
    resources:
      ...
      - qos-policies/status
      - bgp-edge-routers
      - bgp-edge-routers/status
      - bgp-edge-router-advertisements
      - bgp-edge-router-advertisements/status
      - gobgp-configs
      - gobgp-configs/status
    verbs:
      - "*"
  - apiGroups:
      - ""
