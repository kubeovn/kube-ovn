...
func renderFRRConfig() error {
	...
	if holdTime := os.Getenv("BGP_HOLD_TIME"); holdTime != "" {
		seconds, err := parseDurationToSeconds(holdTime)
		if err != nil {
			return fmt.Errorf("failed to parse BGP_HOLD_TIME %q: %w", holdTime, err)
		}
		config.HoldTime = seconds
	}
	if keepaliveTime := os.Getenv("BGP_KEEPALIVE_TIME"); keepaliveTime != "" {
		seconds, err := parseDurationToSeconds(keepaliveTime)
		if err != nil {
			return fmt.Errorf("failed to parse BGP_KEEPALIVE_TIME %q: %w", keepaliveTime, err)
		}
		config.KeepaliveTime = seconds
	}
	if connectTime := os.Getenv("BGP_CONNECT_TIME"); connectTime != "" {
		seconds, err := parseDurationToSeconds(connectTime)
		if err != nil {
			return fmt.Errorf("failed to parse BGP_CONNECT_TIME %q: %w", connectTime, err)
		}
...
func validateConfig(config *Config) error {
	...
	if config.VNI != "" {
		...
		if len(config.RouteTargets) == 0 {
			return errors.New("ROUTE_TARGETS is required when VNI is set")
		}
	}
...
func renderTemplateToFile(templateContent, outputPath string, data any) error {
	...
	f, err := os.OpenFile(outputPath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0o640)
	if err != nil {
